...
function detectSrc(raw) {
  ...
  // "Direct" checké AVANT "Booking" — portail Smoobu envoie "Direct Booking" qui doit être Direct
...
function nextThursday(dateStr) {
  ...
  const p = clean.split('-');
  // Date.UTC évite tout décalage de fuseau horaire (serveur Vercel en UTC)
...
function calcDatePaiement(src, typeNorm, ci, co, dateCreation) {
  ...
  // AIRCOVER et AJUSTEMENT Airbnb → date manuelle uniquement, jamais recalculée
  if (src === 'Airbnb' && (typeNorm === 'AIRCOVER' || typeNorm === 'AJUSTEMENT')) return null;
  if (src === 'Airbnb') {
...
function normalizeApiBooking(booking) {
  ...
  // ── Type normalisation ───────────────────────────────────
  // Règle révisée (2026-08-03) — TOUTE annulation → ANNULATION_NON_PAYEE par défaut.
  // L'ancien critère Airbnb « price-details contient "Cancellation Payout - EUR" »
  // (règle 2026-05-01) est PROUVÉ NON FIABLE : Smoobu affiche cette ligne avec un
  // montant THÉORIQUE (prix − frais hôte) même pour les annulations gratuites —
  // vérifié le 2026-08-03 sur 3 annulations d'août non facturées (faux CA +21k MAD).
  // La promotion en ANNULATION_PAYEE se fait UNIQUEMENT via la file
  // « Annulations à vérifier » du CRM (décision Hakim, override_manual=true)
  // ou la réconciliation (versement réel → mad_reel). Champ resa.annul_verif.
  ...
  // ── Commission ──────────────────────────────────────────
  // Règle CRM : toujours utiliser les taux standards (COM[src]).
  // commission-included de Smoobu est ignoré — Smoobu renvoie des taux
  // variables par réservation (taux négociés, annulations, etc.) qui ne
  // correspondent pas aux règles métier CRM (Airbnb=15,5%, Booking=22%...).
  ...
  if (src === 'Booking.com' && nuitsSejou > 0 && ap) {
    // Taxe due pour CHAQUE personne (adultes + enfants), pas seulement les adultes
