...
| 2026-07-10 | **feat(messages) — carte « À traiter » reliée à la CONVERSATION COMPLÈTE** : retour Hakim — la carte ne montre que le(s) dernier(s) message(s) du voyageur, impossible de juger le brouillon sans le contexte (« améliorer ou enlever »). Décision : améliorer (la file À traiter = système de notification, le badge et l'auto-résolution en dépendent). Nouveau bouton **💬 Conversation** sur chaque carte pending → `msgOpenConv(id)` : ouvre la modale conversation du booking (fil entier traduit + horodaté), **reprend le brouillon de la carte** (édité ou non) dans la zone de réponse (petites tentatives espacées si rendu différé), pose `window._CONV_PENDING={id,booking_id}` APRÈS `convOpenBooking` (convOpen le remet à null à chaque ouverture normale). **`convSend` : un envoi depuis la modale résout automatiquement la carte d'origine** (PATCH `statut='resolved'` + `msgRemoveCard`, best-effort silencieux — si l'écriture échoue, la carte reste et « ✓ Traité » manuel marche toujours). Rappel : l'IA reçoit DÉJÀ le fil complet sur ces cartes depuis P1 (04/07) — le manque était l'affichage côté Hakim. Syntaxe validée (extraction scripts + node --check). |
| 2026-07-10 | **fiche IA Touahri 11 — RÈGLE CLIM CHAMBRE / EAU CHAUDE (SÉCURITÉ GAZ)** : info terrain Hakim, absente de la fiche. L'unité extérieure (ventilateur) de la clim de la CHAMBRE PRINCIPALE (suite parentale) est sur le balcon de la CUISINE, face au chauffe-eau à gaz → quand cette clim tourne, le ventilateur souffle sur le chauffe-eau ; si l'eau chaude est utilisée en même temps (douche) : flamme perturbée + odeur de gaz. Consigne client : éteindre la clim de la chambre principale PENDANT l'eau chaude, la rallumer après ; la clim du SALON (unité sur le balcon du salon) = aucun problème. Odeur de gaz signalée → éteindre clim chambre + aérer + prévenir Hakim. **Ajouté en tête de `logements.kb->regles`** (UPDATE jsonb_set, préfixe « ⚠️ RÈGLE CLIM CHAMBRE / EAU CHAUDE » → invocable par consigne « rappelle-lui la règle de la clim de la chambre »). **globalPlaybook corrigé** : l'ancien rappel générique « éteindre la clim de la chambre pendant l'eau chaude » s'appliquait à TOUS les logements alors que le problème n'existe qu'à Touahri 11 → remplacé par « consignes sécurité clim/eau chaude PROPRES À CHAQUE LOGEMENT, appliquer uniquement celles de la fiche, ne pas inventer pour les autres logements ». |
| 2026-07-10 | **P4 LIVRÉ — KPI qualité IA « % de brouillons envoyés sans retouche »** : migration `add_p4_quality_tracking_to_messages` (colonnes `ai_original` text + `sent_text` text sur `messages`). **Capture** : `?send=1` fige `ai_original = ai_draft_fr\|\|ai_draft` du record AVANT l'écrasement de ai_draft (qui perdait la baseline) + `sent_text = texte envoyé` ; `?sendDirect=1` accepte `ai_original` du frontend + `sent_text`. **Frontend** : baseline `cur.lastAiDraft` posée par convGenerate/convRefine/convReword, `window._MANUAL_LAST_AI` par manualBuildResultHtml/manualRefine/manualReword ; transmise par convSend et manualSendToClient (null si Hakim écrit sans IA → exclu du KPI). **Affichage** : bloc `#msg-kpi` sous le titre Messages IA — `_msgQualityKPI()` (appelée par renderMessages, best-effort) lit les envois `sent` des 30 derniers jours avec ai_original+sent_text non nuls, compare normalisé (espaces/casse), affiche « 🎯 Qualité IA (30 j) : X% · n/N envois » (vert ≥80% / orange ≥50% / rouge) ; masqué tant qu'aucun envoi mesuré (les envois antérieurs à P4 sont exclus, sent_text null). **Stratégie inchangée** : envoi auto des cas « simple » envisageable seulement quand le KPI tient ≥80% plusieurs semaines ; jamais d'auto sur sensible/conflit/remboursement. |
| 2026-07-21 | **feat(lastminute) — AUTOMATISATION LAST-MINUTE (commits `4729cc4`+`828b37d`)** : quand une résa arrive APRÈS les fenêtres des messages auto Smoobu (B « Prepare your arrival » = J-2 12h-13h ; « Departure information » = J-1 21h-22h — les deux « envoi si passé » DÉCOCHÉ, heures constatées dans Smoobu le 21/07), le CRM détecte et envoie lui-même : B-LM immédiatement, infos départ ~1h après (jamais avant la fin sûre de la fenêtre Smoobu — B1=12:45 UTC, D1=21:30 UTC, fenêtres encadrées large car fuseau du compte Smoobu non exposé). **Nouveau `api/lastminute-run.js`** (CJS, smoobuFetch HMAC dupliqué maison) + table `scheduled_messages` (UNIQUE smoobu_id+kind, statuts scheduled/sending/sent/skipped/cancelled/error, RLS read-auth) + forward webhook (new/update/cancel/deleteReservation → `?booking=SID`) + **pg_cron Supabase `nex-lastminute-runner` toutes les 10 min** (pg_net http_post → endpoint ; Vercel Hobby = 2 crons max, déjà pris). GARDE-FOUS : (1) jamais de code d'accès dans les templates ; (2) anti-doublon : verrou unique + claim atomique (PATCH statut=eq.scheduled) + **contrôle conversation Smoobu avant CHAQUE envoi** (regex marqueurs sur messages hôte — couvre les envois Smoobu, le template manuel 1-clic de Hakim, et les runs concurrents) ; (3) revalidation annulation/modification de dates au moment de l'envoi (recalcul complet depuis la résa actuelle ; dates repoussées → replanifié ou rendu à Smoobu) ; (4) **`LAUNCH_EPOCH` 2026-07-21 17:00 UTC** : résas créées avant = skip 'pre-launch' (gérées manuellement, souvent hors conversation). Textes par logement dans `logements.kb.msg_lastminute`/`msg_depart` (miroir des modèles Smoobu validés, placeholders {prenom}/{guide}/{date_depart} ; fallback dans le code) ; `{guide}` = `guest-app-url` du GET /api/reservations/{id} (lien guide + check-in par résa). Trace : chaque envoi = INSERT `messages` statut sent + `client_summary_fr` « 🕐 Envoi auto last-minute — … » (visible fil conversation + Envois récents ; ai_original null → exclu du KPI P4). Diagnostic : `GET ?status=1` (40 dernières lignes), `?dryrun=1` (décisions sans envoi), heartbeat `sync_heartbeat id=lastminute-run`. Vérifié prod : dryrun OK, garde pre-launch OK (cas El Azaar re-scanné → skipped pre-launch). |
| 2026-07-21 | **fix(ia) — RÈGLE CLIM / TÉLÉCOMMANDE (incident réel : l'IA a révélé l'emplacement de la télécommande)** : un client a demandé piscine + télécommande clim ; l'IA a répondu où trouver la télécommande — l'emplacement était écrit dans `logements.kb.faq` (Touahri « armoire à miroir », Agdal « placard cuisine »). CORRIGÉ : emplacements SUPPRIMÉS des fiches + règle ⚡⚡ ajoutée au `globalPlaybook()` ET aux `kb.services` des 4 logements : la clim est un SERVICE PAYANT 3€/nuit → ne JAMAIS indiquer l'emplacement de la télécommande ni laisser entendre qu'on peut l'utiliser ; processus = proposer le service → demande de paiement via la plateforme → paiement accepté et VÉRIFIÉ par Hakim → Hakim active la clim et indique la télécommande. + **Fiches kb alignées sur les fichiers de référence du 21/07** (socle commun + fiches logement) : quiet hours Touahri 21h-09h, non-fumeur PARTOUT à Touahri (balcon inclus), navette 20€/30€ 09h-23h/23h-09h Rabat-Salé uniquement (les 4), serviettes 5€, ménage Agdal 10€ / autres 15€, lit bébé 25€/séjour, animaux = chat OU petit chien <5 kg (1 max), accès Nahda complété (porte fermée après 20h, clé+jeton ascenseur DANS l'appart, 1re montée escaliers, pas de numéro sur la porte), accès Riyad complété (capteur d'empreinte interdit, max 3 essais, clé immeuble DANS l'appart), restitution départ par logement dans `checkout`, piscine Touahri (<3 ans accompagnés), ⚠️ temporaires Nahda (1 WC maintenance, placard fermé chambre 2 lits), adresse Agdal 10090, règlement complet (art. 490, check-in en ligne, clé 25€) dans `regles` des 4. |
| 2026-07-22 | **fix(ia-playbook) — 3 règles « brouillon envoyable sans retouche » (commits `9774668` + `aedd809` + `0da6c8f`, vérifiés prod via `?manualDraft=1`)** — entrée ajoutée rétroactivement le 2026-07-30 (cette session du 22/07 avait déployé sans journaliser dans CLAUDE.md ; code + mémoire OK, seul le journal manquait). (1) **ANNULATION = conditions strictes** appliquées automatiquement (cas Joseph) — voir aussi règles annulation 23/07. (2) **DEMANDE AU-DELÀ DE L'OFFRE** (pas de faux espoir : rappeler le maximum + tarif, jamais « je vois ce que je peux faire ») + **RÈGLE DÉJÀ ANNONCÉE** (règle écrite sur l'annonce → « c'est indiqué avant réservation », jamais d'excuse), cas Naz. (3) **ON RÉPOND TOUJOURS AU VOYAGEUR** (même « merci » → courtoisie brève) + **OFFRE EN SUSPENS** (option payante + réponse vague → relancer pour confirmation, sous réserve de dispo, à l'avance) ; `isTrivialMessage()` neutralisé = seul un message vide est trivial. Méthode Hakim : chaque brouillon faible signalé → RÈGLE PERMANENTE dans `globalPlaybook()` (1 seule copie, pas de règle des 3 copies pour les prompts). ⚠️ Volume d'appels Claude en hausse (les « merci » passent désormais par l'IA) → surveiller le crédit. |
| 2026-07-23 | **fix(lastminute) — vrai lien check-in + envoi après le Welcome (commits `53cb1f5`+`a8c514f`)** : 1er cas réel (ابوباسل, 148589056, résa jour J 11:04 UTC) — mécanique OK (1 seul envoi, départ laissé à Smoobu) mais 2 anomalies : (a) le B-LM présentait `guest-app-url` comme lien de check-in → ce lien ouvre le GUIDE, pas le formulaire. **Fait structurel : le vrai lien = champ `onlineCheckInUrl` de `GET login.smoobu.com/api-guest/bookings/{id}?token={t}`** (t = param de guest-app-url ; endpoint invité public sans clé/HMAC ; URL localisée dans la langue du voyageur — vérifié /ar et /de). Nouveau helper `getOnlineCheckInUrl()` + placeholder `{checkin_url}` distinct de `{guide}` ; les 4 `kb.msg_lastminute` = miroir exact du modèle B-LM Smoobu (2 liens) ; fallback = guide si l'URL est indisponible. (b) B-LM parti 9 s après la résa, AVANT le Welcome Smoobu (12:04 vs 12:07) → `decideBlm` missed-window planifie à création+7 min et le pg_cron passe à `*/5` → envoi effectif **7-12 min** après la résa (cible Hakim : ≤10-15 min, le Welcome part en 2-5 min). ⚠️ Rappel : le CRM envoie SA COPIE des modèles (l'API ne déclenche pas les templates Smoobu) — toute modif d'un modèle Smoobu doit être répercutée dans `logements.kb.msg_*`. |
| 2026-07-23 | **fix(ia) — annulation : refus automatique d'un logement alternatif + fermeté identique à chaque insistance (`c0d4dc9`)** : suite du cas 148589056 (le voyageur demande l'annulation pour « une seule salle de bain », puis un autre appartement « en payant la différence », puis conteste le montant remboursé par Airbnb). Règles ajoutées au bloc ANNULATION du playbook : demande d'un autre logement en contexte d'annulation → réponse AUTOMATIQUE « pas de disponibilité », ⛔ jamais « je vais voir et je reviens vers vous » ; insistance répétée → répéter calmement LA MÊME position à chaque message (conditions de l'annonce, réclamations via l'assistance plateforme, appartement préparé + calendrier bloqué), sans JAMAIS commenter les montants de remboursement (calcul plateforme). Répercuté dans `crm_contexte` (ligne 03). Nota : sur le cas réel, les bonnes réponses avaient nécessité les consignes manuelles de Hakim + régénération — ces règles rendent le 1er brouillon conforme. |
| 2026-07-24 | **fix(annulation) — date_paiement sans date_creation : règle de la source au lieu d'« aujourd'hui » (commit `3e97f14`)** : retour terrain Hakim — en filtrant Réservations sur juillet + type ANNULATION_NON_PAYEE, des annulations d'avril/janvier remontaient avec date de paiement = 24/07 (jour même). **Cause** : 6 annulations non payées (Booking/Airbnb, importées CSV) ont `date_creation = NULL` en base ; la règle `calcDatePaiement` pour ANNULATION_NON_PAYEE était `return dateCreation || t` (t = today) → sans date de création, fallback = aujourd'hui → `recomputeAllPaymentDates()` (appelée au boot) les recollait au **mois courant** à chaque ouverture (elles « flottaient » : juillet aujourd'hui, août le mois prochain) → pollution du filtre du mois en cours. **Fix (3 copies de la règle métier)** : `index.html` calcDatePaiement + `lib/smoobu-normalizer.js` + `api/smoobu-poll.js` → `if (typeNorm === 'ANNULATION_NON_PAYEE' && dateCreation) return dateCreation;` puis on **laisse la règle de la source s'appliquer** (Airbnb checkin+1, Booking jeudi après départ) pour ancrer l'annulation dans son VRAI mois. **Aucun impact CA** (annulations non payées = 0, exclues de tous les KPIs — seul le classement par mois change). **DB : 6 lignes re-datées** (Natalya→2026-01-15, Joel→2026-02-01, Aminata→2026-03-01, Samir→2026-04-16, Usman→2026-04-20, Raphael→2026-04-30). Vérifié prod : nouvelle logique live (index.html L9795) + répartition annulations propre (juillet = 1 seule, Dahmani El Mehdi). ⚠️ Règle des 3 copies respectée. Déployé Vercel READY prod. |
| 2026-07-28 | **fix(messages-ia) — cartes « À traiter » déjà répondues qui traînaient (commit `0555d46`)** : retour terrain Hakim (cas Asmae Khazzane, booking 148949281) — carte pending « Oui, bien passé, merci » (28/07 08:10 UTC) encore affichée à 16h43 alors qu'il avait répondu à 09:14 puis clôturé à 11:15, l'obligeant à « Ignorer » à la main. **Cause** : Smoobu n'envoie pas de webhook fiable pour la réponse de l'HÔTE tapée sur Airbnb → l'auto-résolution (`hostRepliedAfterWh` webhook) ne se déclenche pas, et le sync complet (`?sync=1`, ~400 threads, plafonné) ne tourne qu'au cron quotidien 8h UTC → la carte restait jusqu'au lendemain. **Fix** : nouvel endpoint léger **`GET ?resolvePending=1`** (après le health check) — lit UNIQUEMENT les bookings ayant un pending (`statut=eq.pending`, une poignée au plus), relit chaque fil en direct via `getSmoobuMessages`+`analyzeConversation`, et passe en `resolved` ceux où l'hôte a répondu après le dernier message voyageur (ou aucun message voyageur). Zéro appel Claude, borné par le nb de pending → rapide et fiable. Frontend : `resolvePendingMessages()` (fetch) + `msgOpenView()` (render instantané → passe ciblée → re-render si résolu) branché sur `goTo('messages')` (remplace `renderMessages()` direct) et en tête de `msgRefreshAndSync()` (avant le sync lourd de fond). **Vérifié E2E prod** : carte Asmae remise en pending → `?resolvePending=1` → `{resolved:1,checked:1}` → resolved ; 0 pending restant. Messagerie = 1 seule copie (règle des 3 copies non concernée). Déployé Vercel READY prod. |
| 2026-07-28 | **fix(ia) — clim/télécommande : exception « autorisation explicite de Hakim » + champ fiche confidentiel (commits `0555d46`+`370735d`)** : retour terrain Hakim — il a décidé d'offrir la clim gratuitement à une cliente (Asmae) et a demandé à l'IA d'indiquer la télécommande ; l'IA a refusé car la règle CLIMATISATION du `globalPlaybook()` était **absolue** (« ne JAMAIS indiquer … prime sur toute information d'emplacement »). De plus l'emplacement avait été **supprimé des fiches le 21/07** (incident fuite) → l'IA ne l'a plus. **Fix (1) règle** : la règle CLIM devient « RÈGLE PAR DÉFAUT » + **EXCEPTION** — suspendue quand une INSTRUCTION DE HAKIM demande explicitement d'offrir/activer la clim ou d'indiquer la télécommande (geste commercial OU paiement vérifié côté Hakim) → l'IA OBÉIT et indique l'emplacement (celui donné dans la consigne, sinon champ fiche, sinon message d'activation sans inventer d'emplacement). Jamais de sa propre initiative, jamais sur simple demande du voyageur. **Fix (2) donnée** : `kbBlock()` rend un nouveau champ **`logements.kb.telecommande_clim`** marqué CONFIDENTIEL (révélé au voyageur UNIQUEMENT sur autorisation explicite de Hakim). Décision Hakim : stocker les emplacements par logement (SQL). **✅ Les 4 `kb.telecommande_clim` saisis + validés par Hakim + vérifiés E2E prod** (Touahri = cuisine placard le plus haut au fond ; Agdal = cuisine placard le plus haut au-dessus de la hotte, à l'intérieur ; Riad Ahl Sala = au-dessus du canapé bleu à l'entrée, sur le dessus de la pharmacie à miroir ; Al Boustane = placard d'entrée à gauche, à l'intérieur en haut à droite). ⚠️ Les valeurs récupérées du journal (Touahri « armoire à miroir », Agdal « placard cuisine ») étaient PÉRIMÉES — Hakim avait déplacé les télécommandes. **Règle : toujours faire CONFIRMER un emplacement par Hakim avant de l'écrire.** Déployé Vercel READY prod. |
| 2026-07-28 | **fix(ia) — clim : message PAR DÉFAUT = « activation », jamais de télécommande (commit `58fc211`)** : retour Hakim — avant paiement, l'IA disait « une fois payé je vous indique l'emplacement de la télécommande » → révèle qu'une télécommande est DANS le logement → le voyageur peut la chercher et l'utiliser sans payer. Règle CLIM du `globalPlaybook()` réécrite : message par défaut = clim service optionnel 3 €/nuit (comme indiqué dans l'annonce) **ACTIVÉ une fois le paiement confirmé** ; ⛔ interdiction de mentionner la télécommande ou de laisser entendre qu'un appareil est déjà sur place ; formuler comme une ACTIVATION (« j'active le service une fois le règlement confirmé »). L'emplacement n'est indiqué QUE via l'EXCEPTION (autorisation explicite Hakim : paiement confirmé ou geste commercial). Vaut pour les 4 logements. `crm_contexte` ligne 03 alignée. **Vérifié E2E prod** : défaut Riad Ahl Sala → « j'active le service… » (0 mention télécommande) ; autorisé Al Boustane → emplacement exact donné. Déployé Vercel READY prod. |
| 2026-07-28 | **fix(ia) — MODE DE PAIEMENT des extras + taxe selon la plateforme (commit `2fc0e16`)** : retour Hakim — les demandes de paiement ne valent QUE pour Airbnb ; les clients **Booking paient TOUJOURS en espèces** (euros ou dirhams), en main propre, aussi bien la taxe de séjour que les services additionnels. **Vérification demandée par Hakim (« la règle existe-t-elle déjà ? ») : NON** — absente du playbook messagerie, de `crm_contexte`, des docs de référence, des skills et de la mémoire ; pire, les 4 `kb.services` disaient « demande de paiement via la plateforme » pour TOUS = faux pour Booking (le CLAUDE.md ne l'avait QUE côté compta CRM `pay_source`, que l'IA de messagerie ne lit pas). **Fix** : (1) nouvelle règle maître **MODE DE PAIEMENT** dans `globalPlaybook()` (source unique) — Airbnb = demande via Airbnb, Booking = espèces euros/dirhams, jamais de demande plateforme ; (2) règle clim + OFFRE EN SUSPENS + `hakimStyleGuide` alignées (« paiement selon la plateforme ») ; (3) **4 `kb.services` corrigées (SQL)** : « lui envoyer une demande de paiement via la plateforme » → « selon sa plateforme (Airbnb : demande via Airbnb ; Booking : espèces euros/dirhams, en main propre) ». `crm_contexte` ligne 03 complétée. **Vérifié E2E prod** (même question clim Riad Ahl Sala) : source Booking → « le règlement se fait en espèces (euros ou dirhams) en main propre » ; source Airbnb → « je vous envoie une demande de paiement via Airbnb ». **Complété le 28/07 (commit `1ea5f5f`)** — précision Hakim : pour Booking, AUCUNE demande plateforme quelle que soit la situation ; 3 moyens = (1) espèces euros/dirhams, (2) virement sur le RIB de la société (surtout client marocain), (3) Western Union (surtout client étranger) ; s'applique aussi aux **VOYAGEURS SUPPLÉMENTAIRES / non déclarés** (résa 2 mais ils sont 4 → supplément voyageur réglé par ces moyens, jamais via Booking) ; ⛔ ne jamais inventer de RIB/IBAN (Hakim le fournit). 4 `kb.services` alignées (3 moyens). Vérifié E2E prod (Booking 2→4 + clim → 3 moyens, détection dépassement OK). Déployé Vercel READY prod. |
| 2026-07-28 | **feat(ia) — « répondre uniquement à la demande » + RIB société actif + clients DIRECT (commits `94119cd`+`fad1cca`)** : (1) règle « ⚡ RÉPONDS UNIQUEMENT À LA DEMANDE DU VOYAGEUR » dans `globalPlaybook()` — pas d'upsell ni de catalogue spontané, traiter exactement ce qui est demandé (exception : OFFRE EN SUSPENS). (2) **RIB société** stocké HORS GIT dans `crm_contexte` (id=`rib-societe`) : bénéficiaire **NEX ESTATE**, RIB **050 810 0060115211982001 43**, BIC **CAFGMAMC** ; helper `getCompanyRib()` (cache 5 min) l'injecte dans le prompt de `generateFullAnalysis` avec garde : communiqué UNIQUEMENT à un client **Booking ou Direct** qui règle par virement, OU sur consigne de Hakim ; **jamais à un client Airbnb**, jamais spontané. Ne jamais inventer de RIB. (3) **Moyens alternatifs (espèces/virement/WU) étendus aux clients DIRECT** (hors plateforme, WhatsApp) — le CRM ne détecte PAS les clients directs ni les prospects (Smoobu n'expose que les réservations confirmées Airbnb/Booking) → Hakim colle le message dans IA Manuelle et choisit source=Direct. Airbnb reste sur la demande de paiement via la plateforme (pas de RIB). `crm_contexte` ligne 03 alignée. **Vérifié E2E prod** : Booking+virement → RIB donné (NEX ESTATE + CAFGMAMC) ; Direct+virement → RIB donné ; Airbnb → 0 RIB, demande via Airbnb. Déployé Vercel READY prod. |
| 2026-07-28 | **feat(ia) — PRIORITÉ ABSOLUE : les consignes de Hakim priment sur les règles auto (commit `b6d54e0`)** : retour Hakim — il garde TOUJOURS la décision finale. Règle maître en tête de `globalPlaybook()` : les règles décrivent le comportement SPONTANÉ de l'IA ; dès qu'une INSTRUCTION DE HAKIM est fournie, l'IA l'exécute exactement, même hors cadre ou contraire à une règle « par défaut » (ex. donner le RIB ou une alternative de paiement MÊME à un client Airbnb, offrir un service, indiquer la télécommande, geste commercial). Seuls non négociables (car ils le PROTÈGENT) : ne jamais INVENTER d'info (RIB/code/adresse/tarif/dispo), ni reconnaître un tort en litige. Garde RIB + ligne Airbnb du MODE DE PAIEMENT assouplies (« par défaut » sauf consigne Hakim). `crm_contexte` ligne 03 alignée. **Vérifié E2E prod** : Airbnb + consigne « envoie-lui le RIB » → l'IA obéit (RIB donné) ; Airbnb sans consigne → reste sur demande Airbnb (0 RIB, même si le client réclame le virement). Déployé Vercel READY prod. |
| 2026-08-03 | **fix(annulations) CRITIQUE — annulation = NON_PAYEE par défaut + file « Annulations à vérifier » (commit `be4a293`)** : audit Hakim — CA CRM août 73 998 MAD vs Airbnb 51 971,74 MAD. Cause : la règle 2026-05-01 classait ANNULATION_PAYEE dès que `price-details` contient « Cancellation Payout - EUR » ; **PROUVÉ NON FIABLE par sondes `?probe=` sur les 3 annulations d'août** (Camara 147619036 / Ammar 141177192 / Amelle 146042476) : Smoobu affiche cette ligne avec un montant THÉORIQUE (prix − frais hôte) même pour les annulations GRATUITES (politique modérée, annulées >5 j avant l'arrivée) → +21 366 MAD de faux CA. Aucun champ API ne dit si le client a été débité (`price-paid`= »No » partout) ; seule vérité = relevés de versement (réconciliation). **Nouvelle règle (LES 3 COPIES ✅)** : toute annulation → ANNULATION_NON_PAYEE (0 €, hors CA) par défaut — normalizer + poll (`mapSmoobuBooking`) + index.html (`parseSmoobuRow` : l'ancien critère CSV `prixEur>0` supprimé aussi). **Migration `add_annul_verif_to_resa`** : `resa.annul_verif` (null/'pending'/'done') + `annul_verif_at`. Flag 'pending' posé UNIQUEMENT à la transition vers annulation (webhook upsert+softDelete, poll A-INSERT/orphan/C-NORMAL) — jamais re-posé si déjà ANNULATION_* (décision Hakim conservée). **Guards F2-bis/F3-bis** : ANNULATION_PAYEE existante → jamais rétrogradée en NON_PAYEE par sync/CSV (protège les 27 historiques confirmées mad_reel/verrou) ; softDelete ne promeut plus brut>0 en PAYEE. **UI** : file « ⚠️ Annulations à vérifier » en tête de vue Réservations (`#annul-queue`, `renderAnnulQueue()`) + badge rouge menu (`#annul-badge`) ; carte = voyageur/dates/montant théorique + suggestion **règle des 5 jours** (annul_verif_at vs checkin) ; boutons `annulConfirmPaid(id)` (→ ANNULATION_PAYEE, net=brut×(1−COM[src]), dp règle source, **override_manual=true**, 'done') / `annulConfirmFree(id)` (→ 'done', + neutralisation si héritée PAYEE sans mad_reel). **DB** : les 3 annulations d'août → NON_PAYEE/0 €/Annulé/dp=date_creation/'done' (vérifiées gratuites par Hakim sur Airbnb) → CA août aligné sur Airbnb. Décision Hakim : la réconciliation reste optionnelle — la file est le mécanisme principal (badge impossible à rater), la réconciliation confirme au centime quand elle est faite. |
| 2026-08-10 | **data(fiches-ia) — TOUAHRI 11 : chauffe-eau à gaz REMPLACÉ par un chauffe-eau ÉLECTRIQUE 100 L → règle de sécurité gaz supprimée** (aucun changement de code, uniquement `logements.kb` + `crm_contexte` + docs de référence). Info terrain Hakim : le message de sécurité du 10/07 (« éteindre la clim de la suite parentale pendant l'utilisation de l'eau chaude ») était **un filet non fiable** — en été les clients utilisent la clim en continu et oubliaient la consigne, avec un vrai risque (combustion perturbée, fumée). Hakim a supprimé la cause : plus aucun appareil à gaz à Touahri. **Correction de mémoire au passage : le chauffe-eau à gaz équipait DEUX logements (Touahri 11 + Riad Ahl Sala), ce que les fiches IA n'avaient jamais enregistré** — seul Touahri le mentionnait. **Écritures (Supabase, pas de déploiement)** : (1) `logements.kb` Touahri — bloc « ⚠️ RÈGLE CLIM CHAMBRE / EAU CHAUDE » retiré de `regles`, « RAPPEL SÉCURITÉ » retiré de `services`, `equipements` précise le chauffe-eau électrique 100 L + interdiction explicite de ressortir l'ancienne consigne ; (2) `logements.kb` Riad Ahl Sala — `equipements` mentionne désormais le chauffe-eau à GAZ (seul logement encore au gaz) ; (3) `crm_contexte / 04-automatisations` — modèle manuel Smoobu « Gaz Climatiseur » marqué SUPPRIMÉ. **Inchangé : la clim reste un service payant 3 €/nuit** (emplacement de la télécommande seulement après paiement vérifié, ou sur consigne explicite de Hakim). **Vérifié E2E prod** via `?manualDraft=1` : (a) client Touahri « clim chambre + douche chaude » → l'IA répond « l'eau chaude fonctionne normalement, vous pouvez l'utiliser librement » + clim 3 €/nuit via Airbnb, **zéro consigne d'extinction** ; (b) « odeur de gaz dans la cuisine » → classé `sensible`, « aucun appareil à gaz dans ce logement » + aération + vérification sur place. **Actions restées côté Smoobu (UI, à faire par Hakim)** : supprimer le modèle manuel « Gaz Climatiseur » et retirer la ligne sécurité chauffe-eau du contenu **Services 537621**. **Complément le jour même — CARTE DU GAZ complète (réponse de Hakim, correction importante) :** le test (b) avait fait dire à l'IA « plaque électrique », ce qui était FAUX. Réalité : **plaque de cuisson à GAZ dans 3 logements (Touahri 11, Riad Ahl Sala, Al Boustane** — 4 ou 5 feux selon le logement, on ne précise pas le nombre au client**) ; Agdal 13 = 100 % ÉLECTRIQUE, aucun appareil à gaz ; fours et autres appareils électriques partout ; chauffe-eau à gaz UNIQUEMENT à Riad Ahl Sala** (3 électriques). Écritures : les 4 `kb.equipements` précisent désormais l'énergie de chaque appareil, et un **protocole « ODEUR DE GAZ »** est en tête de `kb.regles` des 3 logements au gaz (aérer, boutons de plaque en position arrêt, pas de flamme, prévenir Hakim immédiatement, traiter comme sensible, **ne rien inventer sur l'installation** — robinet/bouteille/cause) ; `crm_contexte / 02-portefeuille` porte la carte du gaz pour les 3 surfaces. **Re-vérifié E2E prod** : (b') Touahri odeur de gaz → protocole exact, plus aucune affirmation « pas de gaz » ; (c) Agdal « y a-t-il du gaz ? » → « 100 % électrique » (vrai) ; (d) Riad clim + eau chaude → clim 3 €/nuit + « eau chaude produite par un chauffe-eau à gaz ». **Réponses de Hakim, écrites dans les fiches le jour même :** (1) **Touahri avait DEUX bouteilles de gaz** (plaque + chauffe-eau) ; celle du chauffe-eau a été retirée, **il en reste UNE, celle de la plaque** → un défaut d'installation de la plaque peut ENCORE provoquer une odeur ou une petite fuite : le protocole odeur de gaz reste pleinement applicable à Touahri (précision ajoutée en tête de `kb.regles`). (2) **Riad : le chauffe-eau à gaz n'est PAS automatique** — il est laissé **allumé en permanence** et doit le rester ; le client n'a rien à manipuler et ne doit pas l'éteindre ; s'il n'a pas d'eau chaude ou trouve l'appareil éteint → prévenir Hakim, **ne pas faire rallumer par le client** (aucune procédure validée, ne rien inventer). Re-vérifié E2E prod : « dois-je allumer le chauffe-eau avant ma douche ? » (Riad) → « laissé allumé en permanence, rien à manipuler, ne pas l'éteindre » ; « odeur de gaz » (Touahri) → protocole exact. **Puis PROCÉDURES TERRAIN COMPLÈTES dictées par Hakim (même jour, avec captures de la notice Junkers)** : (i) **bouteilles de gaz** — au Maroc pas de robinet mural, on ferme la BOUTEILLE, toujours dans la cuisine dans le placard **sous l'appareil concerné** ; Touahri + Riad = **plastique à clip** (levier du détendeur : **VERT = fermé**, ROUGE = ouvert — confirmé par Hakim, il le fait lui-même), Al Boustane = **métal classique** (molette du dessus, fermer dans le sens des aiguilles d'une montre, à la main) ; **Riad a DEUX bouteilles** (une sous la plaque, une **sous le chauffe-eau** — ne pas confondre) ; bouteille très légère = vide → on envoie quelqu''un la remplacer, **jamais le voyageur**. (ii) **Odeur de gaz en 2 temps** (décision Hakim) : d''abord faire vérifier que les boutons de la plaque sont fermés ; **si l''odeur persiste**, faire fermer la bouteille en attendant l''intervention. (iii) **Rallumage du chauffe-bain Junkers de Riad** (allumage piézo + thermocouple, notice fournie par Hakim) : bouteille ouverte → **⚠️ manette du dessus AU MILIEU (cause n°1 d''échec : à fond à gauche = éteint, à fond à droite = pas d''étincelle)** → appuyer fort et **maintenir 10-15 s** jusqu''à flamme stable → relâcher ; recommencer si elle s''éteint ; milieu = feu modéré (été), à fond à droite = maximum ; **ne jamais parler au voyageur de la molette de débit/température** (réglée par Nex-Estate). Écrit dans `kb.faq` des 3 logements (rendu par `kbBlock` — pas de nouveau champ, donc aucun déploiement) + protocole odeur de gaz mis à jour dans `kb.regles` + `crm_contexte/02-portefeuille`. **⛔ Règle transverse : l''IA ne parle JAMAIS spontanément du gaz, des bouteilles ni du chauffe-eau — uniquement quand le voyageur signale un problème.** Vérifié E2E prod (4 cas) : Riad « plus d''eau chaude » → procédure complète et exacte ; Touahri « boutons fermés mais odeur forte » → `sensible` + fermeture de la bouteille (levier sur le vert, placard sous la plaque) ; Al Boustane « la plaque ne s''allume plus » → test de la bouteille vide + remplacement par nos soins ; **contre-test** (question banale check-out/commerces à Riad) → aucune mention du gaz. **Dernier affinage (retour Hakim, même jour) — nommer les DEUX commandes du chauffe-bain** : il manquait de quoi les distinguer côté voyageur. Écrit dans la fiche : (a) **en HAUT la TOUCHE DE COMMANDE qui GLISSE** de gauche à droite = la seule à manipuler (gauche = éteint, milieu = allumage + feu modéré, droite = maximum) ; (b) **juste EN DESSOUS une MOLETTE RONDE qui TOURNE** = réglage du **débit d''eau** (sens des aiguilles = moins de débit / eau plus chaude, sens inverse = plus de débit / eau moins chaude) — **elle n''allume rien**. Changement de doctrine assumé : on ne se contente plus de « ne pas en parler », l''IA dit explicitement **« ne touchez pas la molette ronde du dessous, elle est déjà réglée par nos soins »**, avec le repère **« celle du haut se GLISSE, celle du bas se TOURNE »**. Vérifié E2E prod : (A) « plus d''eau chaude » → nomme la touche du haut et écarte la molette ; (B) cas piège « j''ai tourné le bouton rond du bas dans tous les sens » → l''IA explique que cette molette ne règle que le débit et renvoie sur la touche du haut. **Confirmé aussi par Hakim : les consignes plaque de cuisson sont IDENTIQUES sur les 3 logements au gaz, seule la fermeture de la bouteille diffère (métal à Al Boustane vs plastique à clip à Touahri/Riad) — c''est déjà l''état des fiches, rien à changer.** **Côté Smoobu, Hakim a fait le nécessaire le 10/08 : consigne retirée et modèle manuel « Gaz Climatiseur » supprimé.** |
| 2026-08-11 | **feat(récap équipe) — détail des ACHATS par mois dans les cartes (commit `f953583`)** : retour terrain Hakim — les passages de ménage en attente ont leur détail dépliable (« ▸ Voir les passages en attente »), mais quand une personne **avance des achats** (consommables, technicien, intervention…) la carte n'affichait **qu'une somme totale** → il fallait descendre dans Dépenses Business et filtrer pour répondre « tu as acheté quoi ce mois ». **3 changements, AFFICHAGE UNIQUEMENT** (aucun calcul, total, KPI ni règle métier touché — pas concerné par la règle des 3 copies) : (1) nouveau helper **`wkExpDetailHtml(rows)`** (juste avant `renderWkCards`) — détail **groupé par MOIS**, mois le plus récent **déplié d'office**, sous-total + nombre de lignes par mois ; chaque ligne = jour `JJ/MM` + libellé (`desc`, sinon catégorie) + montant, puis 2e ligne grise `catégorie · logement · ✅ réglé / ⏳ à rembourser` ; les infos techniques (équipe, payé par, id) passent en **infobulle** (`title`). Tri par `date_reglement || date` comme le reste de la carte. (2) nouveau bloc **« 🛒 Achats & dépenses »** dans la carte, alimenté par `achatRows` = `fmen===membre` et `cat ∉ {Ménage, Avance caisse, Règlement terrain}` sur la période de la carte → total + « ✅ Réglé » / « ⏳ À rembourser » + `<details>` « ▸ Voir le détail des achats ». **Masqué si la personne a une avance caisse** (ses achats sont déjà dans « Détail Utilisé » → pas de double affichage). (3) **« Détail Utilisé »** des avances caisse : l'ancien tableau technique 8 colonnes (Date/Catégorie/fmen/paid_by/Montant/St./Desc./ID, illisible dans une carte de 375 px avec scroll horizontal) remplacé par le **même rendu groupé par mois**. ⚠️ **Cas connu non modifié** : pour une personne QUI A une avance caisse, ses achats `statut='En attente'` restent hors de `adv` (anti-doublon voulu, cf. § 13) et hors de « Utilisé » (qui ne compte que le `Payé`) → ils n'apparaissent dans aucun total ; situation rare (achat avancé sur ses propres deniers alors qu'elle détient déjà du cash caisse), signalée à Hakim, calcul volontairement laissé tel quel. **Vérifié** : `node --check` sur les 2 scripts inline (0 erreur), rendu contrôlé sur données réelles (Amal 8 achats/3 mois, Rehma détail utilisé) — regroupement, sous-totaux, dépliage d'un mois, ellipsis et **zéro débordement horizontal** (page et cartes, desktop + mobile) ; déployé Vercel READY prod, code confirmé en ligne sur `nex-estate-seven.vercel.app`. |
| 2026-08-11 | **fix(récap équipe) CRITIQUE — fausse dette d'avance de caisse + bloc rendu autonome + carte restructurée (commit `3e05716`)** : audit complet demandé par Hakim sur « Récap paiement & avances caisse ». **① BUG DE CALCUL (faux montants affichés à l'écran)** — « Utilisé » de l'avance caisse retombait sur `(r.paid_by \|\| r.fmen)` (« fallback rétrocompatible »). Or **`paid_by` est VIDE sur 100 % de la base** (vérifié : 0 ligne renseignée) → **tous les ménages déjà payés d'une personne étaient comptés comme dépenses faites avec son avance**. Cas réel : Rehma, avance de **60 MAD remise le 15/07** jamais justifiée, affichait « ⚠️ Dépenses > avances **190 MAD** » en juillet (= ses 4 ménages payés 250 − 60) et **1 320 MAD** en mode En cours (= ses 22 ménages depuis mai). Hakim : « je ne savais même pas d'où sortent ces 190 ». **Fix** : « justifié » = **uniquement `paid_by === personne`** (marqueur explicite), plus aucun repli sur `fmen` ; la carte affiche désormais « Avance 60 MAD remise le 15/07 · justifié 0 · **reste 60 à justifier** · depuis 27 j ». **② PÉRIODE NON AUTONOME** — la ligne « Effectué (période) » lisait `filterPer(DB.business,'b')`, c.-à-d. le **filtre de période du HAUT de la page**, alors que tout le reste de la carte suit les flèches du bloc → une même carte affichait **août en en-tête et juillet au milieu** (constaté par Hakim ; vérifié : Amal 10 passages/690 MAD en août, mais « 2 230 MAD · 31 passages » = juillet). Fix : tout suit la période du bloc, nouveau helper **`wkPeriodLabel()`** et le mois est **nommé dans les libellés** (« 📅 Août 2026 » sous le nom). **③ CARTE RESTRUCTURÉE** en 4 blocs homogènes via **`wkCardBlock()`**, chacun avec son détail dépliable groupé par mois (`wkExpDetailHtml`, désormais paramétrable via `opts.labelPaye/labelAtt`) : 🧹 **Ménages** · 🛒 **Achats avancés de sa poche** · 💰 **Encaissé pour Nex-Estate** (taxe de séjour + services additionnels, en déduction) · 🏦 **Avance de caisse**. Puis **UN solde final unique** = ménages dus + achats à rembourser − encaissé (avant : « À payer » ne contenait que le ménage, et la dette d'avance était affichée à part, jamais additionnée). **④ AVANCE DE CAISSE = COMPTE PERMANENT** : calculée **sans aucun filtre de période** (un cash remis il y a 3 mois doit rester visible) + **bandeau d'alerte `WK_BANNER`** en tête du bloc dans TOUS les modes tant qu'elle n'est pas justifiée (demande Hakim : « ça doit me le rappeler jusqu'à ce que je le solde »). **⑤ MODE « EN COURS » = uniquement le non soldé**, sans limite de date ; les blocs entièrement réglés sont masqués ; état vide = « ✅ Rien en attente — tout le monde est soldé ». **⑥ Garde-fou anti-solde-partiel** : hors mode « En cours », un avertissement sous le solde prévient que le montant est limité à la période affichée. **⑦ Cohérence carte ↔ modale Solder** : `getSoldeData` exclut désormais `paid_by === personne` (même filtre que la carte) — avant, la modale pouvait proposer un montant différent de la carte. **⑧ Chaînon manquant corrigé** : le champ « Payé par » (`ff-paid-by`) n'était visible **que pour la catégorie Ménage** → impossible de marquer un achat de consommables comme payé avec l'avance, donc **aucune avance ne pouvait être justifiée**. Il s'affiche désormais sur toutes les catégories à payeur (`CATS_WITH_PAYEUR`, sauf « Avance caisse » elle-même) avec le libellé contextuel « Payé avec l'avance de caisse de ». ⚠️ **Règle métier à retenir** : `fmen` = la personne à qui on doit / qui a fait le travail ; `paid_by` = qui a sorti le cash. **Ne jamais confondre les deux, ne jamais faire retomber l'un sur l'autre.** **Vérifié** en exécutant le **vrai `renderWkCards()`** extrait d'index.html sur les **données réelles de production** (209 lignes business, 37 taxe, 24 serv) dans 3 modes : En cours (Amal 50 / Rabiaa 100 / Rehma 130 + avance 60 à justifier), Août (Amal 690/10 passages — bug corrigé), Juillet (Amal 2 230/31, achats 622/9, « ✅ Soldé ») ; détail par mois, sous-totaux, 0 débordement horizontal à 350 px. Déployé Vercel READY prod. **RESTE À FAIRE (validé par Hakim, prochaine session)** : journal des règlements (aujourd'hui `confirmSolde` ne pose **pas** de `date_reglement` et ne crée une trace « Règlement terrain » **que si le payeur ≠ Hakim** → **aucune traçabilité** de « soldé quand, combien, sur combien de passages ») ; + décision sur l'intégration de l'avance de caisse dans le bouton Solder. |
| 2026-08-11 | **feat(récap équipe) — JOURNAL DES RÈGLEMENTS + avance de caisse séparée du solde (commits `e4517d3` + `5a44c06`)** — points 6 et 7 de l'audit, validés par Hakim. **POINT 6 — décision d'architecture : l'avance de caisse reste un COMPTE SÉPARÉ du règlement.** Raisonnement donné à Hakim : le paiement du travail est un **flux** qui repart à zéro à chaque solde ; l'avance de caisse est un **fonds de roulement** laissé à la personne pour qu'elle achète sans sortir son argent → la déduire d'office au moment de solder reviendrait à **lui reprendre son fonds sans le vouloir** (et c'est exactement le mélange des deux comptes qui avait produit la fausse dette de 190 MAD). Mise en œuvre : (a) **modale Solder** — case à cocher « Récupérer aussi l'avance non justifiée (−X MAD) », **décochée par défaut**, le net se recalcule en direct (`soldeRefreshNet()`, ligne « − Avance de caisse récupérée » ajoutée au récap) ; (b) **carte** — bouton « 🏦 Clôturer l'avance » (`cloturerAvance(name)`) utilisable à tout moment hors règlement, avec confirmation explicite rappelant qu'un achat doit plutôt être saisi via « Payé avec l'avance de caisse de X ». Dans les deux cas la clôture = **écriture inverse** : une ligne `cat='Avance caisse'` de **montant négatif**, datée et tracée, qui ramène le compte à zéro (aucune migration sur `business` ; « Avance caisse » est déjà exclue de `bizRows`/`bizPeriod`/`bPend` → **zéro impact KPI**). **POINT 7 — traçabilité des soldes.** Constat : `confirmSolde` passait les lignes en `Payé` **sans poser `date_reglement`** et ne créait une trace « Règlement terrain » **que si le payeur ≠ Hakim** (donc jamais, puisque c'est Hakim qui paie) → **aucun moyen de savoir soldé quand, combien, sur combien de passages**. ⚠️ Écarté volontairement : poser `date_reglement` au solde ferait **basculer les lignes de période** (`rd() = date_reglement \|\| date` pilote `inRange` dans les cartes) → un ménage de juillet soldé en août serait compté en août. Solution retenue = **table dédiée**. Migration **`create_reglements_journal`** : table `reglements` (personne, date, montant, payeur, periode, nb_menage, tot_menage, nb_achats, tot_achats, tot_taxe, tot_extras, avance_recuperee, ids jsonb, created_at), index `(personne, date desc)`, **RLS calquée sur `business`** (select/insert/update `admin`+`manager`, delete `admin`). `TABLE`/`DB` étendus avec `reglements` → chargée automatiquement par `loadAllDB()`. `confirmSolde` écrit une ligne à chaque solde (**best-effort** : si le journal échoue, le solde reste valide) avec le net **réellement versé** (avance déduite si la case était cochée). Affichage : rappel « 🧾 Dernier règlement : date · montant · N passages » en bas de chaque carte + bloc dépliable « 🧾 Derniers règlements » sous le bloc (`wkReglementsHtml()`). **Bonus corrigé au passage (commit `e4517d3`)** : le champ « Payé par » (`ff-paid-by`) n'était visible **que** pour la catégorie Ménage → aucune avance ne pouvait être justifiée ; il s'affiche désormais sur toutes les catégories à payeur (`CATS_WITH_PAYEUR`, sauf « Avance caisse ») avec le libellé contextuel « Payé avec l'avance de caisse de ». **Vérifié** : vrai `renderWkCards()` **et** vraie modale Solder extraits d'index.html et exécutés sur les données de production — Rehma net 130 MAD, case cochée → **70 MAD** avec la ligne « avance récupérée » ; journal rendu dans les cartes et sous le bloc ; schéma `reglements` validé par un INSERT/DELETE réel (table laissée à 0 ligne). Déployé Vercel READY prod. |
| 2026-08-12 | **fix(business) — `paid_by` n'était JAMAIS enregistré hors Ménage + « Avance caisse » absente du filtre Catégorie (commit `eff830c`)** : deux retours terrain Hakim sur le bloc Ménage & équipe, le lendemain de la livraison du 11/08. **① Le bug qui rendait toute justification d'avance impossible** — le champ « Payé avec l'avance de caisse de X » avait bien été rendu visible sur toutes les catégories à payeur le 11/08 (`e4517d3`, `onBizCatChange`), **mais `saveBiz()` jetait la valeur** : `var paidByName = (cat === 'Ménage') ? gv('fi-paid-by') : ''`. Hakim a saisi son achat de liquide vaisselle (Consommables, 60 MAD, 09/08, « Payé avec l'avance de caisse de → Rehma »), l'écran affichait bien Rehma… et la base a reçu `paid_by = NULL`. L'avance de Rehma continuait donc d'afficher « reste 60 MAD à justifier » avec le bandeau orange, en boucle. **Vérification base : `paid_by` renseigné sur 0 ligne / 349** — le mécanisme n'avait jamais pu fonctionner, ce qui explique aussi la fausse dette corrigée la veille (la cause racine était en amont, à l'écriture). **Fix** : `paidByName` calculé avec **exactement la même condition que l'affichage du champ** (`CATS_WITH_PAYEUR.indexOf(cat)!==-1 && cat!=='Avance caisse'`). ⚠️ **Règle à retenir : une condition d'affichage de champ et la condition de sauvegarde correspondante doivent être écrites une seule fois / rester synchronisées** — ici le formulaire promettait une saisie que l'enregistrement effaçait en silence, sans le moindre message d'erreur. **② « Avance caisse » et « Règlement terrain » introuvables dans le filtre Catégorie** — `MS_CATS_HIDDEN` les retirait du menu déroulant (`msCatBuild`) alors que **les lignes sont bien présentes dans la liste** (`renderBiz` ne filtre pas par catégorie) → impossible de retrouver une avance ou un règlement. Renommé `MS_CATS_INTERNES` et **réaffichés sous un séparateur « ── Mouvements internes ── »**. Zéro impact KPI : `bizRows`/`bizPeriod`/`bPend` les excluent déjà par catégorie, indépendamment du filtre d'écran. **DB réparée** : la ligne `msq9x1iaoy2r` (Consommables 60 MAD, 09/08, Agdal 13) reçoit `paid_by='Rehma'` + `paid_by_id` → **l'avance de Rehma retombe à 0** (reçu 60 − justifié 60), bandeau orange éteint, et l'achat n'est PAS recompté comme avance de sa poche (`achatRows` exclut `paid_by===name`). **Vérifié E2E sur la prod** : code en ligne contrôlé après déploiement (ancienne ligne absente, nouvelle présente) ; **vrai `msCatBuild()` extrait du fichier déployé** → 22 catégories dont Avance caisse et Règlement terrain après le séparateur ; **vraies lignes de calcul de l'avance (7539-7544) extraites du fichier déployé** et rejouées sur les **349 lignes business réelles** → Amal 0/0, Rabiaa 0/0, Rehma reçu 60 / justifié 60 / reste 0 / bandeau non ; « à payer » inchangé (Amal 50 · 1 passage, Rabiaa 100 · 1, Rehma 130 · 2) ; modale Solder Rehma = 2 lignes / 130 MAD, aucun double comptage. Une seule copie de logique (index.html, bloc business) — règle des 3 copies non concernée. Déployé Vercel READY prod. |
| 2026-08-12 | **feat(récap équipe) — RATTACHEMENT AUTOMATIQUE à l'avance de caisse + historique des règlements enfin visible (commit `a4725b2`)** : demande Hakim — *« ce n'est pas à toi de rattacher les achats à chaque fois ; une fois que je saisis, ça doit se rattacher tout seul »*. **① SOURCE UNIQUE DE VÉRITÉ** — nouveau helper **`avanceCaisse(name)`** → `{recu, just, reste, aJustifier, depassement, actif}`. Les **4 copies** du calcul (carte `renderWkCards`, bandeau `avAlert`, `cloturerAvance`, `getSoldeData`) sont supprimées et branchées dessus. Contrôle : il ne reste **qu'une seule occurrence** du filtre dans tout le fichier. C'est la parade structurelle aux dérives déjà vues deux fois sur ce bloc. **② RATTACHEMENT AUTOMATIQUE** — `bizAvanceAutoLink()`, appelée à l'**ouverture** du formulaire Dépenses Business (`openModal`) et à **chaque changement** de catégorie (`onBizCatChange`) ou de personne (`fi-fmen`) : si la personne détient encore du cash de la caisse, le champ « Payé avec l'avance de caisse de » est **pré-rempli d'office** avec son nom, + encart d'explication (`#paid-by-hint`) sous le champ. Hakim garde la main : choisir « — Hakim (direct) — » signifie « payé de sa poche » → remboursé au prochain règlement (le message change en conséquence). **Jamais** sur `Ménage` (là `paid_by` = qui a réglé la prestation, sémantique différente) ni sur `Avance caisse`. Marqueurs sur le select : **`data-touched`** (Hakim a choisi à la main) et **`data-saved`** (valeur déjà en base) → l'automatisme ne recouvre **jamais** un choix explicite. Effet de bord voulu : **ouvrir une ancienne dépense non rattachée la rattache automatiquement** → plus aucune intervention manuelle nécessaire pour l'historique. **③ GARDE-FOU « DÉPASSEMENT » (conséquence directe de ②, sinon on introduit un trou d'argent)** — un achat de 200 MAD rattaché à une caisse de 60 rendait `justifié > reçu` : l'ancien code affichait « Dépassement » mais **les 140 MAD sortis de la poche de la personne n'étaient ajoutés à AUCUN solde**. Désormais `depassement` est **ajouté au net à payer** (carte : `net = menDu + achatDu + avDep − tax − ext` ; modale : ligne « 🛒 Achats au-delà de son avance de caisse »), et `confirmSolde` écrit l'**entrée de caisse symétrique** (`cat='Avance caisse'`, montant **positif** `+avDep`, datée) qui remet le compte à zéro — miroir exact de l'écriture inverse déjà utilisée pour la clôture. **④ HISTORIQUE DES RÈGLEMENTS VISIBLE** — le bloc `wkReglementsHtml()` faisait `if (!rows.length) return ''` : journal vide (0 ligne, créé le 11/08) ⇒ **bloc totalement absent de l'écran**, d'où « je ne le trouve pas ». Il s'affiche maintenant **toujours** : état vide explicatif (dit ce qui s'y écrira et que l'avant-11/08 n'est pas reconstituable), sinon **récap PAR PERSONNE** — nombre de règlements, total versé, **MOYENNE par règlement**, date du dernier — puis le détail des **20 derniers**. Rendu **aussi** dans la branche « tout le monde est soldé » (`!cards.length`), qui l'omettait alors que c'est précisément le moment où Hakim veut le relire. Migration **`add_avance_depassement_to_reglements`** (`reglements.avance_depassement numeric not null default 0`). **Vérifié** : `bizAvanceAutoLink` réel extrait du fichier et rejoué sur **6 cas** (nouvelle saisie → rattaché ; forçage manuel → respecté ; personne sans avance → rien ; Ménage → rien ; Avance caisse → rien ; valeur déjà enregistrée → intacte) ; `avanceCaisse` réel rejoué sur les **349 lignes de production** avec achats de 30 / 60 / 200 MAD → reste 30 / 0 / 0 et dépassement 0 / 0 / **140**, solde Rehma 130 → **270** ; rendu du journal à vide et rempli (moyennes correctes) ; schéma validé par un INSERT/DELETE réel (table laissée à 0 ligne) ; les 11 marqueurs contrôlés sur le fichier réellement en ligne. ⚠️ **Constat signalé à Hakim, non modifié** : `Ayman Lesfer` (membre **inactif**) porte **950 MAD** d'avance de caisse de mai 2026 jamais justifiés ; invisible à l'écran car les cartes et le bandeau ne parcourent que les membres actifs (`DB.team.filter(m => m.active)`). Déployé Vercel READY prod. |
| 2026-08-13 | **fix(récap équipe) — DÉTAIL DE L'AVANCE DE CAISSE : total signé (relevé de caisse) au lieu d'une somme brute (commit `11ec94f`)** : retour terrain Hakim le jour même de son 1er test réel de l'avance de caisse (40 MAD remis à Rabiaa, 6 MAD d'achat « gote parfum » → il doit rester 34). Le bloc affichait bien « Reste 34 MAD à justifier », **mais en dépliant « ▸ Voir le détail de l'avance » le sous-total du mois annonçait « août 2026 · 2 lignes — 46 MAD »** — Hakim : « je peux comprendre que j'ai avancé 46 dirhams, et après dans le détail il y a une contradiction ». **Cause** : `wkExpDetailHtml` faisait `sum(g,'montant')`, or ce bloc est le SEUL des quatre à mélanger des **ENTRÉES** de caisse (`➕ Avance reçue`, +40) et des **SORTIES** (`➖ Consommables`, 6) — les montants sont tous positifs en base, donc la somme brute additionnait 40 + 6 = 46, un nombre qui ne correspond à rien (ni au cash remis, ni au dépensé, ni au reste). Ce n'était PAS une erreur de calcul de l'avance (les totaux 40 / 6 / 34 étaient justes partout ailleurs) mais un **total d'affichage dénué de sens**. **Fix — nouveau mode « relevé » `opts.signe`, activé UNIQUEMENT sur le bloc avance** : (1) chaque ligne porte `sens` (+1 entrée / −1 sortie) et s'affiche **signée et colorée** (`+ 40 MAD` vert / `− 6 MAD` rouge) ; (2) le sous-total du mois devient le **NET du mois** (`+ 34 MAD` suivi d'un petit « net »), et « 2 lignes » devient « 2 **mouvements** » → la somme des nets de tous les mois = le reste en caisse, quelle que soit la répartition (avance en juillet, dépense en août : `+100` / `−30`, total 70) ; (3) **récap en tête du détail** (`opts.entete`), la seule lecture qui ne prête pas à confusion : `💵 Cash remis au total 40 MAD` · `🛒 Dépensé avec cette avance − 6 MAD` · `= Reste en caisse chez Rabiaa 34 MAD` (ou `= A avancé de sa poche X → à lui rembourser` en cas de dépassement) ; (4) en-tête du bloc : le montant est désormais annoncé comme **« 40 MAD · remis le 13/08 »** (c'était déjà le cash remis, ce n'était simplement pas dit). ⚠️ **AFFICHAGE UNIQUEMENT** — aucun calcul, total, KPI ni règle métier touché ; `avanceCaisse()` (source unique de vérité du 12/08) est inchangée ; les **3 autres blocs** (🧹 Ménages, 🛒 Achats, 💰 Encaissé) n'utilisent pas `signe` et rendent exactement comme avant (vérifié). Règle des 3 copies non concernée (index.html uniquement). ⚠️ **Règle à retenir : ne jamais additionner des montants de sens opposés dans un même total** — soit on sépare (entrées / sorties), soit on signe. Le CRM stocke tous les montants en positif, le sens vient de la catégorie (`Avance caisse` = entrée) ou du marqueur `paid_by` (= sortie de caisse). **Vérifié** : `node --check` sur les 2 scripts inline (0 erreur) ; **vrai `wkExpDetailHtml` extrait du fichier** et rejoué sur les données réelles (Rabiaa : `46 MAD` avant → `+ 34 MAD` après ; multi-mois +100/−30 → nets `+100` et `−30` ; dépassement 60/200 → `− 140 MAD` ; bloc Ménages non signé → `130 MAD` identique à avant) ; **vrai `renderWkCards()` rejoué sur les 356 lignes business de production** (Amal 260 · Rabiaa 200 · Rehma 190 — soldes inchangés) ; puis **re-rejoué sur le fichier RÉELLEMENT EN LIGNE** après déploiement (récap + lignes signées + « 2 mouvements » présents). Déployé Vercel READY prod. |
| 2026-08-13 | **fix(récap équipe) CRITIQUE — un achat rattaché à l'avance et laissé « En attente » disparaissait de TOUS les totaux (commit `edaa395`)** : Hakim teste le nouveau circuit (Consommables 60 MAD, Rabiaa) et signale que « le système n'a pas comptabilisé les 60 en utilisant l'avance de caisse ». **Ce qu'il avait réellement enregistré était juste** (`paid_by` vide + `En attente` = sa méthode habituelle → les 60 partent bien dans « 🛒 Achats avancés de sa poche », à payer 260) — **mais la capture montrait la modale rouverte avec le rattachement automatique du 12/08 ayant pré-rempli « Payé avec l'avance de caisse de → Rabiaa »**, statut toujours « En attente ». **En enregistrant dans cet état, les 60 MAD s'évaporaient** : hors du bloc avance (`just` exigeait `statut='Payé'`) ET hors du bloc achats (`achatRows` exclut `paid_by === personne`) → « À payer » retombait de 260 à **200**. C'est exactement l'angle mort signalé le 11/08 (« situation rare, calcul laissé tel quel ») — sauf que **le rattachement automatique du 12/08 l'a rendu atteignable en un clic**, sur la saisie la plus banale. ⚠️ **Leçon : un angle mort qu'on accepte parce qu'il est « rare » doit être re-jugé dès qu'une automatisation le met sur le chemin par défaut.** **3 correctifs.** ① **`avanceCaisse()` — `just` ne regarde PLUS le statut** : `paid_by = la personne` **est** la preuve que l'argent est sorti de la caisse (le cash est parti le jour de la remise), le statut de la ligne n'y change rien. Aucun risque de double comptage : carte, bloc Achats et modale Solder restent mutuellement exclusifs sur le seul critère `paid_by`. `recu` garde `statut='Payé'` (règle 2026-05-03 : une avance non remise n'est pas du cash). ② **`saveBiz` — statut forcé à « Payé » quand `paid_by` est renseigné hors Ménage** : « payé avec l'avance » + « En attente » est un état contradictoire (on ne doit plus rien pour cette ligne). Le Ménage est exclu du forçage — `paid_by` y signifie « qui a réglé la prestation », autre sémantique (cf. flux `confirmSolde` §13). ③ **Formulaire — libellés qui disent ce qu'ils font** : l'option vide devient **« — Non : payé de sa poche (à lui rembourser) — »** hors Ménage (« — Hakim (direct) — » ne voulait rien dire à l'usage et c'est pourtant le cas COURANT chez Hakim), et l'encart `#paid-by-hint` énonce la conséquence des deux choix (rattaché → déduit de l'avance + ligne « Payé » + rien de plus dû ; de sa poche → s'ajoute au solde, l'avance ne bouge pas). **Rappel de la méthode de travail de Hakim (à préserver) : il donne rarement du cash d'avance ; son circuit normal = catégorie + « Payé par / avancé par » = la personne + statut « En attente » → la dépense arrive dans « Achats avancés de sa poche » et se règle au bouton Solder.** L'avance de caisse ne doit intervenir QUE lorsqu'il a réellement remis du cash. **Vérifié** sur les données réelles de production avec le vrai `renderWkCards()` : (a) méthode habituelle (`paid_by` vide, En attente) → **inchangée**, Rabiaa 260 = 200 ménages + 60 achats, avance toujours « reste 34 à justifier » ; (b) scénario du piège (`paid_by=Rabiaa`, En attente) → **avant 200 MAD (60 disparus), après 226 MAD** = 200 ménages + 26 de dépassement, détail « 40 remis / 66 dépensés / a avancé 26 de sa poche → à lui rembourser », carte ↔ modale Solder cohérentes (`getSoldeData` passe par `avanceCaisse()`) ; (c) Amal 260 et Rehma 190 inchangés ; (d) `node --check` sur les 2 scripts inline. **Re-vérifié E2E sur le fichier RÉELLEMENT EN LIGNE** après déploiement (260 / 226 identiques). ⚠️ GitHub a renvoyé des `500 Internal Server Error` sur le push alors que **la référence était bien mise à jour** — toujours vérifier avec `git fetch` + comparaison `HEAD` / `origin/main` avant de conclure à un échec et de re-pousser. Déployé Vercel READY prod. |
| 2026-08-13 | **fix(business) — l'avance de caisse était comptée comme une DÉPENSE dans les totaux de l'écran + `fmen` renseigné d'office + lisibilité du solde (commits `9a65ccf` + `462bda1`)** : après son test réussi (achat rattaché → « À payer 226 »), Hakim pose deux questions de fond. **① « L'activité de 60 est passée à Payé alors que la caisse ne couvrait que 34 — ça ne perturbe pas les calculs ? »** → le forçage du statut est correct (le fournisseur est réglé à 100 %, ce qui reste est une **dette envers la personne**, suivie dans sa carte et dans le compte de caisse, pas une facture en attente). **MAIS la vérification a révélé un vrai double comptage, ailleurs et bien antérieur** : `renderBiz` sommait **TOUTES** les lignes de la période pour `b-tot`/`b-men`/`b-chg`/`b-att`/`b-split`, **y compris `Avance caisse` et `Règlement terrain`**. Or ces deux catégories ne sont pas des dépenses — le cash remis en avance est déjà compté le jour où la personne l'utilise (c'est l'**achat** la dépense), et un règlement terrain paie des lignes déjà comptées. **C'est la règle §13, appliquée depuis toujours aux KPIs du Dashboard (`bizRows`/`bizPeriod`/`bPend`) mais JAMAIS aux totaux de cet écran.** Mesuré sur août 2026 : **24 499 MAD affichés pour 24 459 MAD de vraies dépenses** (40 MAD d'avance comptés EN PLUS des 66 MAD d'achats qu'ils financent) — l'écart grandit à chaque avance et à chaque règlement. **Fix** : les totaux portent sur `rowsTot` (hors mouvements internes) + mention explicite **« ↔️ hors X MAD de mouvements internes (N lignes) »** sous le split payé/en attente ; les lignes restent **visibles dans la liste** et retrouvables via le filtre Catégorie (acquis du 12/08, non remis en cause). ⚠️ **Leçon : une règle d'exclusion écrite pour « les KPIs » doit être vérifiée sur TOUS les endroits qui affichent un total, pas seulement sur ceux qui portent le mot KPI.** **② « Que je remplisse la case femme de ménage ou pas, ça ne change rien ? »** → **vrai dans un cas, faux dans l'autre**, vérifié en rejouant `renderWkCards` sur les données réelles : (a) achat **rattaché à la caisse** (`paid_by` rempli) → `fmen` ne change effectivement rien, c'est `paid_by` qui pilote (`avanceCaisse` ne lit que lui) → carte identique à 226 MAD ; (b) achat **payé de sa poche** (`paid_by` vide) **et `fmen` vide** → la dépense **n'apparaît dans AUCUNE carte** (Rabiaa retombe à 200 MAD, les 60 ne sont réclamés par personne). C'est l'état exact dans lequel Hakim avait laissé la ligne (`msrm5bwe5ttb`, `fmen=''`). **Fix** : `saveBiz` renseigne `fmen = paid_by` quand `paid_by` est posé hors Ménage et que `fmen` est vide — la ligne reste rattachée à la personne dans le filtre Équipe, la recherche « Lié à » et l'historique, et surtout elle ne peut plus devenir orpheline si le rattachement est retiré ensuite. **③ LISIBILITÉ (demande explicite : « le plus simple des calculs, facile à lire »)** : nouvelle ligne sous le solde qui écrit le calcul en toutes lettres — **« 🧮 200 MAD de ménages + 60 MAD d'achats avancés »** (− taxe / services encaissés le cas échéant), affichée seulement s'il y a plus d'une composante ; et le jargon du bloc avance disparaît — « Justifié 6 MAD · Reste 34 à justifier » devient **« Dépensé 6 sur 40 MAD · Il lui reste 34 MAD de ta caisse »**, et en dépassement **« Elle a mis 26 MAD de sa poche → ajouté à ce que tu lui dois »**. **⚠️ Nuance assumée et signalée à Hakim** : une dépense réglée avec l'avance passe en « Payé », donc la part encore due à la personne (le dépassement) n'apparaît PAS dans le « ⏳ en attente » de Dépenses Business — elle est suivie dans le Récap équipe (« À payer ») et dans le compte de caisse. Deux questions différentes : *Dépenses Business* = les factures à régler, *Récap équipe* = ce qu'on doit aux gens. Aucun montant n'est perdu. **Vérifié** : `node --check` ; bloc réel de `renderBiz` rejoué sur les **48 lignes d'août** → Total 24 459, Ménage 1 485 + Charges 22 974 = Total, En attente 11 885, mention « hors 40 MAD (1 ligne) » ; `renderWkCards` réel sur les 2 cas de `fmen` ; **re-vérifié sur le fichier RÉELLEMENT EN LIGNE** après déploiement (mêmes chiffres). Déployé Vercel READY prod. |
| 2026-08-13 | **feat(récap équipe) — JOURNAL DES RÈGLEMENTS : recherche, filtre par personne, pagination (commit `6d6a17d`)** : 1er solde réel de Hakim (Rabiaa, **200 MAD, 2 passages**, écrit dans `reglements` à 15:45 UTC — le journal créé le 11/08 a donc sa première ligne). Sa question : *« je garde ça combien de temps ? c'est cumulé jusqu'à l'infini ? et quand ça dépassera 5 ou 10 règlements, est-ce que je peux faire une recherche ? est-ce que c'est la meilleure chose qu'on a actuellement ? »*. **Réponse factuelle** : la table `reglements` conserve **tout, à vie** — aucune purge, aucune limite de durée, et rien ne la vide jamais (c'est de l'historique financier, cf. l'ajout au backup complet le jour même). **Mais l'écran, lui, était limité** : `wkReglementsHtml` faisait `all.slice(0, limite||20)` sans aucun moyen de chercher ni de dépasser 20 → **au-delà de 20 règlements, les plus anciens devenaient inatteignables depuis l'interface** (ils restaient en base, donc récupérables en SQL, mais invisibles). Réponse honnête donnée à Hakim : ce n'était pas suffisant. **Livré** : (1) **champ de recherche** `#wk-reg-q` — recherche **par MOTS** (tous les mots doivent matcher) sur personne + date (`2026-08` comme `13/08/2026`) + montant + période + payeur, **accents ignorés** (`wkRegNorm`) → « rabiaa 08 » = ses règlements d'août ; (2) **clic sur une personne** du récap = filtre sur elle, **reclic = retour à tout le monde**, ligne active surlignée (`✓` + fond `--ors`) ; (3) **pagination** « ▾ Afficher 20 de plus » + « Tout afficher (N) » (ce dernier seulement s'il reste > 20) ; (4) le **récap par personne et son total suivent la recherche** (« Par personne — 10 règlements trouvés · 1 515 MAD »), tandis que **l'en-tête garde toujours le total GLOBAL** (« N règlements · X MAD versés au total ») — sinon on ne sait plus ce qu'on regarde ; (5) **état « aucun résultat »** avec lien « Tout afficher ». **Détail d'implémentation qui compte** : la fonction est scindée en `wkReglementsHtml()` (enveloppe `<details>` + summary + **champ de recherche rendu UNE fois**) et **`wkRegBodyHtml()`** (récap + détail) ; `wkRegFilter/wkRegPerson/wkRegMore` ne re-rendent que **`#wk-reg-body`** → **le champ garde le focus et la position du curseur** pendant la frappe (un re-render global du bloc aurait fermé le `<details>` et vidé le focus à chaque lettre). Le `<details>` s'ouvre d'office quand une recherche est active. État conservé dans `WK_REG_Q`/`WK_REG_N` (globales) → survit aux re-render de `renderBiz()` (après un Solder par exemple). **AFFICHAGE UNIQUEMENT** — aucune donnée, aucun calcul, aucun KPI touché. **Vérifié** sur **30 règlements** (le vrai + 29 simulés sur 3 personnes et 3 mois), avec le **vrai code extrait du fichier** : 20 affichés + bouton « Afficher 10 de plus » ; « Tout afficher » → 30 lignes rendues et bouton retiré ; « rabiaa » → 1 seule personne + surbrillance ; « 2026-07 » → 10 trouvés / 1 515 MAD ; « rabiaa 08 » (2 mots) → 1 trouvé / 200 MAD ; « zzz » → message + lien de réinitialisation ; `<details>` fermé par défaut. **Re-vérifié à l'identique sur le fichier RÉELLEMENT EN LIGNE** après déploiement. Déployé Vercel READY prod. |
| 2026-08-13 | **feat(récap équipe) — EXPORT CSV du journal des règlements (commit `db8f269`)** : demandé par Hakim (« pour ouvrir l'historique dans Excel / le donner à un comptable »). Bouton **« ⬇️ CSV »** à droite du champ de recherche du journal. **Exporte ce qui est à l'écran** : tout l'historique par défaut, ou **uniquement les règlements filtrés** si une recherche est active — le nom du fichier porte alors le suffixe `-filtre` (`nex-estate-reglements-2026-08-13-filtre.csv`) pour qu'un export partiel ne soit jamais confondu avec un export complet. **14 colonnes** : Date · Personne · Montant versé · Payeur · Période · Nb passages · Total ménages · Nb achats · Total achats · Taxe déduite · Extras déduits · Avance récupérée · Achats hors avance · Enregistré le. Réutilise le helper existant **`_csvDownload`** (BOM UTF-8 + séparateur `;`) → s'ouvre directement dans Excel FR sans écran d'import ; dates en `JJ/MM/AAAA` et **décimales à la virgule** (`167,5`) pour qu'Excel FR les traite comme des nombres. **Vérifié** sur 6 règlements (le vrai + 5 simulés couvrant taxe déduite, avance récupérée et achats hors avance) : en-têtes, accents, BOM présent, décimales FR, export filtré à 2 lignes avec le bon nom de fichier ; **re-vérifié sur le fichier RÉELLEMENT EN LIGNE** après déploiement. ⚠️ **Question d'affichage laissée OUVERTE par Hakim** (il a explicitement demandé une proposition avant toute action) : le journal s'accumule à vie et le bloc Récap manque de place, surtout sur mobile → 3 pistes soumises (pagination numérotée dans le bloc / **fenêtre dédiée + pagination à l'intérieur, recommandée** / hauteur fixe avec défilement). **Ne rien implémenter avant sa réponse.** Déployé Vercel READY prod. |
| 2026-08-14 | **feat(récap équipe) — le journal des règlements passe dans une FENÊTRE DÉDIÉE + pages numérotées (commit `ed3d55e`)** : Hakim a posé la bonne question d'ergonomie — *« si c'est cumulable jusqu'à l'infini, comment ça se fait au niveau de l'affichage ? je n'ai pas trop d'espace dans le tableau… il faut numéroter par pages ? dis-moi d'abord ce que vous proposez avant de passer à l'action »*. ⚠️ **Méthode : il a explicitement demandé une proposition AVANT toute action** — 3 options lui ont été soumises (pagination dans le bloc / **fenêtre dédiée** / hauteur fixe + défilement), il a choisi la **fenêtre dédiée**. **Problème réel mesuré** : le journal ouvert pouvait empiler **jusqu'à 20 règlements × 2 lignes ≈ 40 lignes** sous les 3 cartes du Récap — intenable sur mobile où le bloc est déjà un carrousel, et le journal grossit à vie. **DANS LE BLOC — résumé de 2 lignes** : « 🧾 Historique des règlements — N règlements · X MAD versés au total », puis les **3 derniers** en une ligne compacte (`13/08 Rabiaa 200 MAD · 12/08 Amal 260 MAD · …`), puis 2 boutons **📜 Historique complet** et **⬇️ CSV** (celui-ci exporte TOUT depuis le bloc). **DANS LA FENÊTRE** (`#reg-ov`, même patron que `plat-fix-ov`/`conv-ov`, z-index 906) : recherche + récap par personne cliquable + **détail PAGINÉ — `wkRegPageSize()` = 10 par page sur ordinateur, 5 sur mobile** — navigation « ‹ Préc. · 1 2 3 4 · Suiv. › » (**fenêtre glissante de 5 numéros** + raccourcis vers la 1re et la dernière page avec « … »), libellé « Détail — 31 à 37 sur 37 » + « Page 4 sur 4 », et l'export CSV (filtré si une recherche est active). Fermeture : bouton ✕, clic hors de la fenêtre, ou **Échap** (ajouté au handler global, après `cmdk` et `conv-ov`). **Garde-fous** : la page se **recale** si un filtre réduit la liste (page 9 → « Page 2 sur 2 ») ; `openRegModal` repart toujours d'une vue propre (`WK_REG_Q=''`, page 1) ; `closeRegModal` remet le filtre à zéro **pour que le bouton CSV du bloc exporte bien tout l'historique** et jamais un reliquat de recherche. `wkRegBodyRefresh()` ne re-rend que `#wk-reg-body` → le champ de recherche garde focus et curseur pendant la frappe. **AFFICHAGE UNIQUEMENT** — aucune donnée ni calcul touché. **Vérifié** avec le vrai code sur **37 règlements** simulés : bloc compact (2 lignes + 2 boutons) ; page 1 = « 1 à 10 sur 37 · Page 1 sur 4 » ; page 4 = « 31 à 37 » ; mobile = 8 pages de 5 ; filtre depuis la page 9 → recalé en 2/2 ; 4 règlements → **aucune** pagination ; 0 règlement → état vide explicatif conservé. **Re-vérifié à l'identique sur le fichier RÉELLEMENT EN LIGNE** après déploiement (9 marqueurs présents). Un **aperçu HTML statique** (bloc + fenêtre avec 25 règlements) a été envoyé à Hakim pour qu'il juge le rendu futur, la prod n'ayant qu'un seul règlement réel à ce stade. Déployé Vercel READY prod. |
| 2026-08-14 | **fix(business) — la colonne « Payé par » du tableau INVENTAIT une valeur (repli sur `fmen`) + le vrai solde d'Ayman Lesfer (commit `7066656`)** : Hakim filtre l'année sur « Avance caisse » + « Solde équipe déjà comptabilisé », voit **« Payé par : Ayman Lesfer » sur toutes les lignes** et conclut légitimement *« je pense que tout est réglé »*. **Or `paid_by` est NULL sur 100 % de ces lignes en base** (vérifié : 0 ligne rattachée sur les 10 d'Ayman) — l'écran affichait un **repli**. Ancien code (L7349) : `paid_by || (cat!=='Ménage' ? fmen : '') || 'Hakim'` → sur une ligne non rattachée, la colonne **ré-affichait la colonne « Femme ménage » d'à côté** (valeur dupliquée, jamais saisie), donc **une avance de caisse non justifiée avait l'air rattachée à la personne**. ⚠️ **C'est EXACTEMENT le piège de la fausse dette du 11/08, cette fois à l'affichage : `paid_by` ne doit jamais retomber sur `fmen`, ni au calcul NI à l'écran.** La carte mobile était déjà correcte (badge 💳 conditionnel) — les deux rendus disaient donc des choses différentes. **Fix** : la colonne montre `paid_by` s'il existe, sinon **« — »** avec infobulle (« réglé directement par Hakim, ou avancé de sa poche et remboursé au règlement »). Vérifié sur 5 cas puis **sur le fichier en ligne**. **VRAI SOLDE D'AYMAN LESFER (chiffré en base, mai 2026)** : cash reçu **950** (4 avances) − versé à l'équipe **790** (4 « Règlement terrain » : Rabiaa 260, Amal 207,5 ×2, Touria 115, créés par `confirmSolde` quand le payeur ≠ Hakim) − achats **92** (Maintenance 80 + Consommables 12) = **68 MAD** réellement non justifiés — et non 950. Le CRM affiche 950 parce que **`avanceCaisse()` ne compte comme « justifié » que `paid_by = la personne`**, et qu'aucune de ces lignes n'est marquée. ⚠️ **Question de fond soulevée, NON tranchée** : `avanceCaisse()` exclut `Règlement terrain` de la justification (`cat!=='Règlement terrain'`, hérité de la règle anti-double-comptage §13) — or **un « Règlement terrain » dont `fmen` = la personne EST du cash sorti de sa caisse** (elle a payé l'équipe avec l'argent reçu). Sans lui, le compte de caisse d'un payeur terrain est structurellement faux. Cas dormant aujourd'hui (Hakim est toujours le payeur → aucun « Règlement terrain » n'est plus créé), donc **rien n'a été modifié dans le calcul** : options soumises à Hakim (a) écriture de clôture datée pour solder l'historique d'Ayman, (b) faire compter les « Règlement terrain » de la personne comme justification. Décision à lui. Déployé Vercel READY prod. |
| 2026-08-14 | **data(avance caisse) — CLÔTURE de l'avance d'Ayman Lesfer (décision Hakim, aucun code touché)** : après avoir vu le calcul détaillé (950 reçus − 790 versés à l'équipe − 92 d'achats = 68 réellement non justifiés), Hakim tranche : **Ayman était un STAGIAIRE parti au bout de moins d'une semaine, réglé par un « Solde de tout compte » de 190 MAD le 06/05/2026 → tout est soldé pour lui**. Écriture de clôture insérée en base (option A validée) : `id='mst0ayman0clo'`, `cat='Avance caisse'`, `fmen='Ayman Lesfer'`, `montant=-950`, `date=2026-08-14`, `statut='Payé'`, `scope='global'`, description portant **le détail complet du calcul et la date de validation** (miroir exact de ce que fait le bouton « 🏦 Clôturer l'avance », indisponible ici car le membre est inactif → les cartes ne parcourent que les actifs). **Vérifié** : `sum(montant) where cat='Avance caisse' and fmen='Ayman Lesfer'` = **0** → `avanceCaisse('Ayman Lesfer').actif = false`, plus aucune avance ouverte dans le CRM. **Zéro impact KPI** (catégorie exclue partout) **et zéro impact sur les totaux de l'écran Dépenses Business** — grâce au correctif de la veille (`462bda1`), sans lequel cette écriture de −950 aurait fait **baisser** le « Total période » d'août de 950 MAD. ⚠️ **Contrôle de cohérence fait au passage** : les « vraies dépenses » d'août étaient passées de 24 459 à 24 499 MAD depuis la veille — **expliqué intégralement par les saisies de Hakim** (ligne de test « test » 60 MAD supprimée `−60`, nouveau ménage Rabiaa 100 MAD du 13/08 `+100`), aucune anomalie. **Question de fond toujours ouverte** (voir entrée précédente) : `avanceCaisse()` ne compte pas les « Règlement terrain » d'une personne comme justification de son avance — dormant tant que Hakim est le seul payeur, non modifié. |
| 2026-08-14 | **feat(équipe) — LE COMPTE DE CAISSE REDEVIENT JUSTE + état des caisses permanent + rôles affichés + nom sur les salaires (commit `eb1d02b`)** : Hakim conteste le chiffre d'Ayman — *« en mai, à chaque fois qu'il payait un technicien, des consommables ou une femme de ménage, je voyais l'avance de caisse déduite. Pour moi c'était correct. Est-ce qu'il y a eu une modification entre-temps ? »*. **VÉRIFIÉ DANS L'HISTORIQUE DU CODE : il a raison.** L'ancien calcul (§13, avant le 11/08) faisait `depNonMen = fmen === name && cat !== Ménage && cat !== Avance caisse` → les 4 « Règlement terrain » d'Ayman (790) **et** ses 2 achats (92) étaient déduits → l'écran affichait bien **950 − 882 = 68**. Le fix du 11/08 (`3e05716`, fausse dette de 190 MAD sur Rehma) a resserré sur `paid_by` seul et **a emporté ce cas légitime au passage** → Ayman est repassé à 950. ⚠️ **Leçon majeure : une correction juste peut supprimer un comportement juste — vérifier l'historique du calcul AVANT d'affirmer qu'un chiffre a toujours été faux.** **① `avanceCaisse()` élargi** : sortie de caisse = **(a)** dépense marquée `paid_by = X` **OU (b)** « Règlement terrain » versé par X (`fmen = X`, cash qu'elle a remis à l'équipe à la place de Hakim). ⛔ **Ce qui ne revient JAMAIS : les prestations que X a elle-même effectuées** (`cat='Ménage' && fmen===X`) — c'était exactement le bug des 190 MAD. **Analyse 360° faite avant d'écrire** (exigence Hakim, cf. mémoire dédiée) : `achatRows`, `bizPend` (modale Solder) et les KPIs excluent tous `Règlement terrain` **par catégorie** → périmètres disjoints, aucun double comptage possible. **DB alignée dans le même mouvement** : les 2 achats d'Ayman (`moosydg1ergp` 80, `moracr22twyz` 12) reçoivent `paid_by='Ayman Lesfer'`, et **sa clôture de la veille passe de −950 à −68** — sans quoi la nouvelle règle l'aurait affiché « on lui doit 790 ». Contrôle : reçu net 882 = 790 versés + 92 achats → **reste 0**. **② Bloc « 🏦 État des caisses »** (remplace le bandeau) : il balaie les personnes depuis **les lignes de caisse elles-mêmes**, plus depuis `members` (= actifs) — c'est ce filtre qui a rendu les 950 MAD d'Ayman **invisibles trois mois**. Affiche les **deux sens** (« X MAD de ta caisse chez elle/lui » / « X MAD avancés de sa poche → tu lui dois »), le **rôle**, la mention **« ancien membre »**, et un état vide explicite quand tout est à zéro. **③ Rôles enfin visibles** — ils existaient en base (`team_members.role`, Ayman = `field_agent`) et étaient saisis depuis la fiche Équipe **depuis le début**, mais n'étaient affichés NULLE PART : d'où le malentendu « je suis obligé de mettre Ayman dans femme de ménage ». `ROLE_LABELS` / `roleLabel()` / `memberLabel()` sortis en **source unique** (ils étaient enfermés dans `renderTeam`), et nouveau `buildMemberOptions()` qui **groupe les menus par métier** (`── Agents terrain ──`) avec le libellé « Ayman Lesfer — Agent terrain ». ⚠️ **`option value` reste le NOM SEUL** (le rôle est un suffixe d'affichage) — sinon toutes les données seraient cassées ; et un nom enregistré absent de l'équipe reste sélectionné (rouvrir une vieille ligne ne l'efface pas). **④ Colonne « Femme ménage » → « Équipe »** ; les filtres **Équipe** et **Payeur** incluent désormais les **anciens membres** sous `── Anciens membres ──` (jusqu'ici impossible de filtrer sur quelqu'un de parti — le besoin exprimé par Hakim : « pouvoir chercher facilement un ancien agent »). `fmenMember`/`paidByMember` n'exigent plus `active` (l'id est conservé à la ré-ouverture). **⑤ `CATS_BENEFICIAIRE` = `Salaires / Équipe` + `Charges sociales / CNSS`** : champ **« Bénéficiaire »** enfin disponible (avant, aucun champ personne → Hakim devait écrire le nom dans la description, ce qu'il a fait pour le solde de tout compte d'Ayman). ⚠️ **Exclus de `achatRows` ET de `bizPend`** — sans ça un salaire « En attente » serait compté comme « achat avancé de sa poche » et le bouton **Solder l'aurait payé EN PLUS des ménages (double paiement)**. `paid_by` masqué sur ces catégories, **condition de `saveBiz` alignée sur celle de l'affichage** (règle du 12/08), `bizAvanceAutoLink` ne s'y déclenche jamais. Flux de Hakim inchangé : saisie « En attente » avec les charges fixes, puis « Payé ». **Vérifié** sur les données réelles : `avanceCaisse` → Ayman 882/882/**0** inactif · Rabiaa 40/6/**34** · Rehma 60/60/0 **avec ses 4 ménages NON déduits** (non-régression explicite du bug des 190) ; `renderWkCards` → Amal 260 · Rabiaa 100 · Rehma 190 ; bandeau réel (Rabiaa 34 · Femme de ménage) et scénario (ancien membre 300 **visible**, technicien 150 « avancés de sa poche »). **Re-vérifié à l'identique sur le fichier RÉELLEMENT EN LIGNE** + 5 marqueurs. Déployé Vercel READY prod. |
| 2026-08-14 | **feat(équipe) — EXCEPTION ASSUMÉE : régler un ménage avec l'avance de caisse d'une collègue + AUDIT DE COHÉRENCE COMPLET (commit `de76975`)** : demande Hakim — *« la règle des ménages non déduits, d'accord, mais je veux garder le CHOIX quand je décide d'utiliser l'avance pour payer. Je veux toujours l'accès, pas limité. »*. **C'était déjà possible** (champ « Payé par » sur Ménage, liste = tous les membres actifs) **mais il manquait le garde-fou** : la ligne restait « En attente », donc **déduite de la caisse de la collègue ET encore affichée comme due à la femme de ménage** → elle aurait été payée **DEUX FOIS** au bouton Solder. Miroir exact du trou du 13/08 (où l'argent disparaissait au lieu d'être payé en double). **Fix** : (1) `saveBiz` — un « Payé par » renseigné force le statut **« Payé »**, **Ménage inclus désormais** (quelqu'un a déjà réglé la ligne) ; (2) `bizAvanceAutoLink` — nouvelle branche Ménage qui **n'écrit RIEN** (le choix reste 100 % manuel, aucun automatisme n'est jamais posé sur un Ménage) mais **explique la conséquence** sous le champ : ligne passée en Payé, déduite de la caisse de X, et au-delà du cash détenu la différence est **due à X** ; (3) condition d'affichage initiale de `ff-paid-by` alignée sur `CATS_BENEFICIAIRE`. ⚠️ **La règle « les ménages de X ne sortent pas de la caisse de X » est INTACTE** : elle vise le **repli AUTOMATIQUE sur `fmen`** (bug des 190 MAD du 11/08), pas un choix explicite de Hakim — vérifié, les 4 ménages de Rehma ne déduisent toujours rien. **Analyse d'impact 360° faite AVANT d'écrire** (exigence Hakim, cf. mémoire dédiée) : `menPendRows`/`bizPend` excluent le statut Payé → plus de double paiement ; la neutralisation de `confirmSolde` (L8340) ne porte que sur les lignes **en attente** → ces lignes n'y passent plus, la déduction de caisse survit ; aucun « Règlement terrain » ne peut les couvrir (déjà payées, donc hors solde) → pas de double comptage de sortie de caisse ; `renderGainsCards` ne filtre pas sur le statut → gains ménage inchangés ; côté KPI la ligne bascule « en attente » → « payé », ce qui est **vrai**. **Vérifié sur données réelles** : Rabiaa règle 30 MAD du ménage d'Amal → Rabiaa 40 reçu / 36 sorti / **reste 4**, Amal 0/0/0 (son ménage ne touche pas SA caisse) ; règle 100 avec 34 en caisse → **66 MAD dus à Rabiaa** ; re-vérifié sur le fichier en ligne. **── AUDIT DE COHÉRENCE DEMANDÉ PAR HAKIM (« confirme-moi que tout est cohérent avant que je recrute ») ──** **(a) Preuve mécanique d'absence de double comptage** : les **4 périmètres** (Ménages dus · Achats avancés · Caisse entrée · Caisse sortie) recopiés du code déployé et appliqués à **366 lignes × 8 personnes = 2 928 combinaisons**, incluant un jeu couvrant TOUS les cas de figure (exception ménage, ménage dû, achat de sa poche, achat avec caisse, salaire, CNSS, règlement terrain, avance) → **0 collision** : aucune ligne ne peut tomber dans deux blocs. **(b) Audit des données** : 0 ligne rattachée mais non payée (état contradictoire impossible depuis le 13/08) ; 0 membre sans rôle ; **une seule caisse ouverte** (Rabiaa 34 MAD) ; 0 ménage portant un « Payé par » (l'exception n'a jamais servi). **Seul point relevé** : 1 ligne orpheline `2026-08-06 Maintenance 10 MAD` (« frais de taxi Mohamed plombier ») sans personne ni payeur — légitime si Hakim l'a réglée lui-même, à compléter seulement si quelqu'un l'a avancée. Déployé Vercel READY prod. |
| 2026-08-14 | **fix(mobile) — 2 débordements qui auraient COUPÉ des montants sur téléphone (commit `570bec1`)** : Hakim — *« je paie toujours la femme de ménage depuis mon mobile, je n'ouvre pas de PC »* — demande de vérifier que les changements des 2 derniers jours tiennent sur téléphone **avant** de clôturer le bloc. **Vérification faite, 2 vrais défauts trouvés dans du code écrit AUJOURD'HUI** : le bloc « 🏦 État des caisses » et le récap par personne du journal mettaient une **phrase entière** en `white-space:nowrap` (« 34 MAD de ta caisse chez elle/lui », « 12 450 MAD · moy. 1 038 MAD ») avec, à gauche, un nom **sans ellipsis**. À 375 px ça déborde — et comme le CRM applique `#app,.content{overflow-x:clip}` en mobile (fix du 18/06 contre l'élargissement de page), le débordement est **COUPÉ, pas rendu défilable** → **le montant devenait invisible, sans aucun signe visuel**. Sur l'écran qui sert à payer les gens, c'est le pire endroit possible. **Fix (même patron pour les deux)** : nom tronqué proprement (`min-width:0` + `text-overflow:ellipsis`), **SEUL le montant en `nowrap`** (contenu borné, ~12 car. max), et la phrase explicative renvoyée sur sa propre ligne en 10 px. ⚠️ **RÈGLE RETENUE POUR CE BLOC : `nowrap` est réservé aux contenus BORNÉS (montant, date, bouton) — jamais une phrase.** **Contrôle structurel automatisé** écrit et passé sur tout le bloc Récap + journal (`mobcheck.js`) : chaque `nowrap` doit être borné (ellipsis, `fmt()`, date ou bouton) → **0 non borné** (2 faux positifs = montants portés par une variable) ; **0 largeur fixe > 300 px** ; **0 `<table>`** (un tableau ne rentre pas sur mobile). Re-passé sur le fichier EN LIGNE après déploiement. **Déjà conformes par ailleurs, vérifiés au passage** : cartes en 1 colonne (`.cg` en `grid-template-columns:1fr` <700 px), carrousel 1 membre à la fois, journal en fenêtre dédiée avec **5 lignes par page sur mobile** (`wkRegPageSize()`), modals en bottom-sheet pleine largeur, inputs 16 px (anti-zoom iOS), lignes de détail déjà à ellipsis (vérifiées à 350 px le 11/08). ⚠️ **Leçon de méthode : le rendu mobile ne se déduit pas d'un rendu desktop qui « a l'air bien » — sur ce CRM, `overflow-x:clip` transforme un débordement en INFORMATION MANQUANTE silencieuse. Tout élément ajouté au Récap équipe doit passer le contrôle des `nowrap` bornés.** Déployé Vercel READY prod. |
| 2026-08-14 | **feat(mobile) — l'historique des règlements SUIT la personne affichée dans le carrousel (commit `5a789e4`)** : retour terrain Hakim **depuis son téléphone** (capture à l'appui) — sur mobile le bloc affiche **une carte à la fois** (carrousel, depuis le 12/06), mais le journal en dessous restait **GLOBAL** : il lisait « 13/08 **Rabiaa** 200 MAD » sous la carte d'**Amal**. *« Amal n'est pas concernée par cet historique… j'aimerais que sur la carte d'Amal on ait l'historique d'Amal, sur Rabiaa celui de Rabiaa, pas un mélange. »* Constat juste : sur desktop les 3 cartes sont côte à côte donc un journal global a du sens, mais en vue mono-personne c'est du bruit qui prête à confusion. **Fix** : `wkReglementsHtml(personne)` — nouveau paramètre optionnel qui filtre le journal, adapte le **titre** (« 🧾 Historique des règlements — Amal »), le **total** (« 350 MAD versés » sans « au total ») et l'**état vide dédié** (« aucun règlement enregistré pour Amal — il s'écrira au premier clic sur 💳 Solder » + bouton Historique complet). **Détail qui compte** : sur une vue mono-personne, répéter son nom à chaque ligne est inutile → remplacé par le **nombre de passages** (« 13/08 · 2 passages · 200 MAD »), l'information réellement utile. **Câblage** : nouveau tableau `cardNoms[]` construit **en parallèle de `cards`** (poussé uniquement quand une carte est réellement produite, donc toujours aligné après le `.filter(Boolean)`) → `perPage === 1` (mobile) passe `cardNoms[start]`, sinon aucun nom = global. **Bouton CSV du bloc** : exporte cette personne quand le bloc est filtré, avec un nom de fichier dédié (`nex-estate-reglements-rabiaa-2026-08-14.csv`) — cohérent avec la règle posée la veille (« le CSV exporte ce qui est à l'écran »). **« 📜 Historique complet » reste GLOBAL** dans tous les cas, avec sa recherche et sa pagination. **Analyse d'impact 360°** (exigence Hakim) : desktop ≤ 5 cartes et branche « tout est soldé » → aucun nom passé, **comportement inchangé** ; affichage seul, aucun calcul / KPI / donnée touchés ; le filtre du bloc passe par un **paramètre**, jamais par `WK_REG_Q` → l'état de recherche de la fenêtre n'est pas pollué ; `cardNoms` poussé **après** le `return ''` des membres sans rien à afficher. **Vérifié** sur 3 règlements / 3 personnes : desktop 3 règlements / 660 MAD ; Rabiaa 2 / 350 ; Rehma 1 / 310 ; Amal état vide dédié — plus un contrôle automatique confirmant **aucune fuite du nom d'une autre personne** dans les 3 vues. **Re-vérifié à l'identique sur le fichier EN LIGNE**. ⚠️ **Précision métier donnée par Hakim au passage** : la ligne orpheline `Maintenance 10 MAD du 06/08` n'est PAS une anomalie — c'est un **technicien EXTERNE** (Mohamed, payé au passage, pas membre de l'équipe) à qui Hakim a donné des frais de taxi après une visite. **Aujourd'hui Hakim n'a AUCUN agent de terrain : c'est lui qui paie tous les techniciens externes**, d'où l'absence légitime de personne sur ce type de ligne. Déployé Vercel READY prod. |
| 2026-08-14 | **fix(mobile) — vue « ⏰ À suivre » : les lignes DÉBORDAIENT et étaient COUPÉES sur téléphone (commit `da56719`)** : retour Hakim — *« j'avais essayé de l'utiliser depuis le mobile et ce n'était pas vraiment compatible »*. Vue jamais auditée jusqu'ici. **Mesuré** : chaque ligne empilait **7 éléments INSÉCABLES sur une seule rangée** (pastille · voyageur · appartement · source · date · montant · bouton Modifier), en `display:flex` **sans `flex-wrap`**. À 375 px il faut déjà **~407 px AVANT même le nom du voyageur** (8 pastille + 60 de gaps + 62 bouton + 75 montant + 72 date + 60 source + 70 appartement) → le nom (`flex:1`) est écrasé à zéro **et le reste déborde quand même** ; avec `#app,.content{overflow-x:clip}` en mobile, le débordement est **COUPÉ, pas rendu défilable** → Hakim voyait des lignes tronquées sans pouvoir lire le montant ni la date. Exactement le même mécanisme que les 2 débordements corrigés le matin même dans le Récap équipe — **c'est un défaut de PATRON, pas un accident isolé**. **Fix** : (1) les 3 infos secondaires (appartement · source/mode · date) sont regroupées dans **UN SEUL `.as-meta`** — c'était impossible à replier tant que c'étaient 3 spans `nowrap` distincts ; (2) **grille mobile** (`@media(max-width:700px)`) en **2 rangées** : `[case] [pastille] [nom…] [Modifier]` puis `[appartement · source · date…] [montant]` (`grid-template-areas`, `.as-meta` repasse en `white-space:normal`) ; **desktop strictement inchangé** (le `display:flex` reste, la grille ne s'active que sous 700 px) ; (3) `rowResa` et `rowSel` étaient **deux copies du même HTML** → fusionnées dans un **`asRow()`** commun + helper `asMeta()` (une seule copie à maintenir désormais). **Analyse d'impact 360°** : les **id des cases à cocher `as-cb-TYPE-ID` sont INCHANGÉS** → les actions groupées (`document.querySelectorAll('input[id^="as-cb-PREFIX-"]')`) et le « tout sélectionner » (`as-cb-PREFIX-all`) continuent de fonctionner — c'était le vrai risque de cette refonte ; les boutons Modifier gardent leur `openModal(type,id)` ; affichage seul, aucun calcul / KPI / donnée touchés ; seule différence visible sur desktop : les 3 infos secondaires sont séparées par « · » au lieu d'espaces (même ligne, même ordre). **Vérifié** : `node --check` ; contrôle structurel (0 `nowrap` non repliable, 0 `<table>`, les 7 classes de grille présentes, media query présente, id de checkbox conservé) ; **rendu réel des 2 types de lignes** (réservation à encaisser avec un nom long « KADI PASCALINE SOUMAHORO » + taxe non reversée) confirmant une **zone meta unique**. Re-vérifié à l'identique **sur le fichier EN LIGNE**. ⚠️ **RÈGLE GÉNÉRALISÉE À TOUT LE CRM : une rangée d'infos sur une seule ligne flex sans `flex-wrap`, avec plusieurs `nowrap`, est un débordement garanti en mobile — et `overflow-x:clip` le transforme en INFORMATION MANQUANTE SILENCIEUSE. Toute vue en liste doit prévoir un repli en 2 rangées sous 700 px.** Déployé Vercel READY prod. |
| | **📌 RESTE : ✅ P4 LIVRÉ le 2026-07-10** (voir entrée dédiée) — le KPI « 🎯 Qualité IA (30 j) » se remplit au fil des envois réels ; **observer plusieurs semaines** avant toute décision d'envoi auto. Option restante : compteur coût IA du mois dans le CRM. **Stratégie Hakim** : valider TOUS les messages manuellement d'abord ; envoi auto (cas « simple » uniquement) seulement quand le KPI tient ≥ 80 % sur plusieurs semaines ; jamais d'auto sur sensible/conflit/remboursement. **Billing** : crédit prépayé Anthropic (pas d'abonnement) ; ~3,39 € au 2026-07-04, pas d'auto-reload → surveiller (à zéro l'IA s'arrête en silence, le CRM continue ; message d'erreur explicite depuis le 10/07). Console : platform.claude.com → Billing / Usage. |
