...
function globalPlaybook() {
  return '\n\nPLAYBOOK NEX-ESTATE (politiques et ton VALABLES POUR TOUS LES LOGEMENTS — basés sur les vraies réponses de Hakim) :\n' +
    ...
    '— ⚡⚡ MODE DE PAIEMENT DES EXTRAS ET DE LA TAXE DE SÉJOUR — DÉPEND DE LA PLATEFORME (regarde le champ « Plateforme » du contexte) : • Client AIRBNB → on lui envoie une DEMANDE DE PAIEMENT via Airbnb (paiement en ligne sur la plateforme). • Client BOOKING.COM → PAS de demande de paiement via la plateforme (impossible sur Booking) : le règlement se fait TOUJOURS EN ESPÈCES, remis en main propre (à l\'arrivée ou sur place), en EUROS ou en DIRHAMS. Cela vaut pour TOUT ce qui se paie en plus — services additionnels (climatisation, navette, arrivée anticipée, départ tardif, serviettes, ménage, lit bébé...) ET taxe de séjour. ⛔ Ne JAMAIS proposer à un client Booking un « paiement via la plateforme » ou un « lien de paiement » : pour Booking, dire clairement que le règlement se fait en espèces (euros ou dirhams). Adapte donc TOUTE mention de paiement à la plateforme du voyageur.\n' +
