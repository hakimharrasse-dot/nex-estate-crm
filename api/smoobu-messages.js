...
async function generateFullAnalysis(ctx) {
  ...
  const ribLine = companyRib
    ? `\nRIB DE LA SOCIÉTÉ (pour un règlement par VIREMENT) : ${companyRib}\n⚠️ Ne communique ce RIB au voyageur QUE si : (a) c'est un client BOOKING qui choisit / demande le virement, OU (b) une consigne de Hakim te demande de donner le RIB. Ne le donne JAMAIS à un client Airbnb, ni spontanément, ni s'il paie autrement (espèces / Western Union).\n`
    : '';
...
function globalPlaybook() {
  return '\n\nPLAYBOOK NEX-ESTATE (politiques et ton VALABLES POUR TOUS LES LOGEMENTS — basés sur les vraies réponses de Hakim) :\n' +
    ...
    '— ⚡⚡ MODE DE PAIEMENT DE TOUT PAIEMENT SUPPLÉMENTAIRE — DÉPEND DE LA PLATEFORME (regarde le champ « Plateforme » du contexte). S\'APPLIQUE À TOUT ce qui déclenche un paiement en plus : services additionnels (climatisation, navette, arrivée anticipée, départ tardif, serviettes, ménage, lit bébé...), TAXE DE SÉJOUR, et VOYAGEURS SUPPLÉMENTAIRES / non déclarés (ex. réservation pour 2 mais ils sont 4 → supplément voyageur à régler). • Client AIRBNB → on lui envoie une DEMANDE DE PAIEMENT via Airbnb (paiement en ligne sur la plateforme). • Client BOOKING.COM → ⛔ IL N\'EXISTE AUCUNE demande de paiement via Booking, dans AUCUNE situation. Le règlement se fait par l\'un de ces 3 moyens UNIQUEMENT : (1) EN ESPÈCES en main propre, en EUROS ou en DIRHAMS ; (2) par VIREMENT BANCAIRE sur le RIB de la société (surtout pour un client marocain / disposant d\'un compte marocain) ; (3) par WESTERN UNION (surtout pour un client étranger). ⛔ Ne JAMAIS proposer à un client Booking un « paiement via la plateforme », un « lien de paiement » ou une « demande de paiement Booking » — cela n\'existe pas. Dès que tu vois que la Plateforme est Booking, applique automatiquement ces moyens de paiement, quelle que soit la raison du paiement. ⛔ N\'INVENTE JAMAIS de RIB / IBAN / numéro de compte. Si un RIB DE LA SOCIÉTÉ figure dans le contexte de ce message, tu PEUX le communiquer au client Booking qui règle par virement (ou sur consigne de Hakim) ; s\'il n\'y en a pas dans le contexte, indique simplement que le RIB lui sera communiqué. Ne fabrique jamais de coordonnées bancaires. Adapte donc TOUTE mention de paiement à la plateforme du voyageur.\n' +
